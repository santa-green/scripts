USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_CorpRep_4_1]    Script Date: 20.06.2019 15:19:29 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[ap_CorpRep_4_1] (
  @BDate SMALLDATETIME,
  @EDate SMALLDATETIME,
  @PartID TINYINT,
  @Ours VARCHAR(MAX),
...
/*
  ...
  5 - Группы товаров, по которым возврат превышает продажу. Сортировка: ORDER BY rpg1.Notes,rpg.PGrName (по возрастанию в алфавитном порядке название проекта + по возрастанию в алфавитном порядке название группы)
*/

/*  
  EXEC ap_CorpRep_4_1 '20180101','20181231',1,'1-5,11','1-100','',''
  EXEC ap_CorpRep_4_1 '20180101','20181231',2,'1-5,11','700','81-99','50-79,2000-3000'
  
  EXEC ap_CorpRep_4_1 '20180101','20181231',3,'1-5,11','1-100','',''
  EXEC ap_CorpRep_4_1 '20180101','20181231',3,'','','',''

  EXEC ap_CorpRep_4_1 '20180101','20181231',4,'1-5,11','1-100','50-79',''
  EXEC ap_CorpRep_4_1 '20180101','20181231',5,'1-5,11','1-100','50-79',''
*/
  ...
  BEGIN
    ...
    INSERT #TCR_4_1
    (GPGrName1, PGrName, InvSumMC, InvQty, ESumMC, EQty, TurnRatio)                    
    SELECT 
     rpg1.Notes GPGrName1, rpg.PGrName, SUM(InvSumMC) InvSumMC, SUM(InvQty) InvQty,SUM(ESumMC) ESumMC,SUM(EQty) EQty,
     ((ROUND(SUM(BQty),0) + ROUND(SUM(EQty),0)) / 2 / (CASE ROUND(SUM(InvQty),0) WHEN 0 THEN NULL ELSE ROUND(SUM(InvQty),0) END)) TurnRatio
    FROM (
          --"На начало"
          SELECT rp.PGrID1,rp.PGrID,SUM(m.Qty * tp.CostMC) BSumMC,SUM(m.Qty) BQty,0 InvSumMC,0 InvQty,0 ESumMC,0 EQty
          ...
          GROUP BY rp.PGrID1,rp.PGrID
          
          UNION ALL
          --Сумма отгрузок за период          
          --Расходная накладная          
          SELECT rp.PGrID1,rp.PGrID,0 BSumMC,0 BQty,SUM(d.Qty*tp.CostMC) InvSumMC,SUM(d.Qty) InvQty,0 ESumMC,0 EQty
          ...
          SELECT rp.PGrID1,rp.PGrID,0 BSumMC,0 BQty,0 InvSumMC,0 InvQty,SUM(m.Qty * tp.CostMC) ESumMC,SUM(m.Qty) EQty
          ...
          WHERE (@Ours = '' OR m.OurID IN (SELECT AValue FROM dbo.af_FilterToTableByTable(@Ours,'r_Ours'))) 
            AND (@PCats = '' OR rp.PCatID IN (SELECT AValue FROM dbo.af_FilterToTableByTable(@PCats,'r_ProdC'))) 
            AND m.StockID NOT BETWEEN 800 AND 899                       
          GROUP BY rp.PGrID1,rp.PGrID ) q
    JOIN r_ProdG1 rpg1 WITH(NOLOCK) ON rpg1.PGrID1 = q.PGrID1
    JOIN r_ProdG rpg WITH(NOLOCK) ON rpg.PGrID = q.PGrID                                                                    
    GROUP BY rpg1.Notes,rpg.PGrName
    ...
    SELECT 
     GPGrName1,PGrName,InvSumMC,InvQty,ESumMC,EQty,
...
-- rkv0 '2019-06-19 11:50' меняю сортировку в каждом модуле на сортировку по наименованию проекта и внутри проекта по наименованию группы/товара от А до Я согласно заявке Танцюры Галины от 19.06.2019.
  ...
  BEGIN                  
    SELECT 
     rpg1.Notes GPGrName1,rpg.PGrName,SUM(BSumMC) BSumMC,SUM(BQty) BQty,SUM(RecSumMC) RecSumMC,SUM(RecQty) RecQty,
    ...
    FROM (
          --"На начало"
          SELECT rp.PGrID1,rp.PGrID,SUM(m.Qty * tp.CostMC) BSumMC,SUM(m.Qty) BQty,0 RecSumMC,0 RecQty,0 InvSumMC,0 InvQty,0 ESumMC,0 EQty
          FROM dbo.af_CalcRemDByRemIM (@BDate) m
          JOIN r_Prods rp WITH(NOLOCK) ON rp.ProdID = m.ProdID
          JOIN t_PInP tp WITH(NOLOCK) ON tp.ProdID = m.ProdID AND tp.PPID = m.PPID
          WHERE (@Ours = '' OR m.OurID IN (SELECT AValue FROM dbo.af_FilterToTableByTable(@Ours,'r_Ours'))) 
          ...
          --Приход за период
          --Приход товара          
          SELECT rp.PGrID1,rp.PGrID,0 BSumMC,0 BQty,SUM(d.Qty*tp.CostMC) RecSumMC,SUM(d.Qty) RecQty,0 InvSumMC,0 InvQty,0 ESumMC,0 EQty
          ...
          --Списание за период
          --Расходный документ          
          SELECT rp.PGrID1,rp.PGrID,0 BSumMC,0 BQty,0 RecSumMC,0 RecQty,SUM(d.Qty*tp.CostMC) InvSumMC,SUM(d.Qty) InvQty,0 ESumMC,0 EQty
          FROM t_ExpD d WITH(NOLOCK)
          JOIN t_Exp m WITH(NOLOCK) ON m.ChID = d.ChID    
          JOIN t_PInP tp WITH(NOLOCK) ON tp.ProdID = d.ProdID AND tp.PPID = d.PPID
          JOIN r_Prods rp WITH(NOLOCK) ON rp.ProdID = d.ProdID
          ...
          --Остатки на конец периода          
          --На конец          
          SELECT rp.PGrID1,rp.PGrID,0 BSumMC,0 BQty,0 RecSumMC,0 RecQty,0 InvSumMC,0 InvQty,SUM(m.Qty * tp.CostMC) ESumMC,SUM(m.Qty) EQty
          FROM dbo.af_CalcRemDByRemIM (@EDate) m
          JOIN r_Prods rp WITH(NOLOCK) ON rp.ProdID = m.ProdID
          JOIN t_PInP tp WITH(NOLOCK) ON tp.ProdID = m.ProdID AND tp.PPID = m.PPID
          WHERE (@Ours = '' OR m.OurID IN (SELECT AValue FROM dbo.af_FilterToTableByTable(@Ours,'r_Ours'))) 
    ...
    ORDER BY rpg1.Notes, rpg.PGrName
  END
