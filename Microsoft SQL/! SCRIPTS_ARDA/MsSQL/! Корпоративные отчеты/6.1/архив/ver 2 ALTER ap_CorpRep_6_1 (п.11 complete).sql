...
SELECT 14 'PartID',0 Style,'Лицензии' 'PartIDName', PGrID1, ROUND(SUM(SumMC),0) 'SumMC' FROM (
          ...
          SELECT rp.PGrID1, 1000 + SUM(0-(t_RecD_145.Qty * t_PInP_146.CostMC)) 'SumMC'
          ...
          JOIN av_t_RecD t_RecD_145 WITH (NOLOCK) ON (t_Rec_1.ChID = t_RecD_145.ChID)
          JOIN r_Deps r_Deps_241 WITH (NOLOCK) ON (r_Deps_241.DepID = t_Rec_1.DepID) -- r_Deps Справочник подразделений
      ...
      WHERE
          (SELECT [dbo].[zf_MatchFilterInt](t_Rec_1.OurID,@OurID,',')) = 1
          AND (SELECT [dbo].[zf_MatchFilterInt](t_Rec_1.CodeID1,@CodeID1_license,',')) = 1
          AND (SELECT [dbo].[zf_MatchFilterInt](t_Rec_1.CodeID4,@CodeID4_license,',')) = 1
          AND (SELECT [dbo].[zf_MatchFilterInt](t_Rec_1.CodeID5,@CodeID5_license,',')) = 1
          GROUP BY rp.PGrID1
