USE [Alef_Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EDI_METRO_KPK_Recadv]    Script Date: 20.11.2020 15:44:04 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<Проверка новых уведомлений RECADV по сети METRO с отправкой email уведомлений>
-- =============================================
...
DECLARE @period tinyint = 4
    ...
    WHERE DocType = 5000 --RECADV
      AND InsertData >= CAST(CAST(GETDATE() - @period as date) as datetime) 
      AND [Status] != 17
      AND RetailersID = 17
    ...
    --отправляем письмо-уведомление о новом RECADV по ручнику.
	    BEGIN
            ...
            DECLARE @query nvarchar(max) = 'SELECT ID ''№ уведомления о приеме'', CONVERT(varchar, InsertData, 104) + '' '' + LEFT(CONVERT(varchar, InsertData, 108),5) ''Дата и время'' FROM [Alef_Elit].[dbo].[at_EDI_reg_files] WHERE DocType = 5000 AND InsertData >= CAST(CAST(GETDATE() - ' + cast(@period as varchar) + ' as date) as datetime) AND [Status] != 17 AND RetailersID = 17 AND Notes NOT LIKE ''4%'''
            
            SET @subject = 'Новые уведомления от сети METRO по ручным заказам КПК: ' + @id_recadv;
            --[CHANGED] rkv0 '2020-09-14 17:27' изменил тело email уведомления.
            --SET @body_str = 'Номера и дату/время новых уведомлений см. во вложении. Что с этим делать? - "Специально обученный сотрудник" отдела автоматизации дальше вручную создаст и отправит документы по системе EDI (товарную накладную и налоговую). [[В перспективе мы это полностью автоматизируем]]';
            --SET @body_str = 'Номера и дату/время новых уведомлений см. во вложении.' + char(10) + char(13) + ' (!) Товарная и налоговая накладная будут сформированы отделом автоматизации, подписаны и отправлены через сайт EDI. Вы получите об этом соответствующее уведомление.';
            --[CHANGED] rkv0 '2020-09-15 13:36' изменил TEXT на HTML.
            SET @body_str = '<body style="background-color:powderblue;">
                Номера и дату/время новых уведомлений см. во вложении. <p> <b> <font color="red"> (!) Товарная и налоговая накладная будут сформированы отделом автоматизации, подписаны и отправлены через сайт EDI. Вы получите об этом соответствующее уведомление. </font> </b> </p>
                </body>';
