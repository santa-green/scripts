...
BEGIN --блок проверок.
    ...
    WHERE rc.TaxPayer = 0 --закладка "Общие данные": галочка "Плательщик НДС".
        ...
        AND EXISTS 
            (SELECT * FROM Elit.dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE()) --закладка "Договора": "действующий" договор, конечная дата > сегодня.
