CREATE PROCEDURE [dbo].[ap_OP_SendXML_Velike_Puzo] (@Testing Int = 0)
AS
...
DECLARE @ID int, @ChID int, @recipients varchar(250) 
IF @Testing = 0 SET @recipients = 'Novom_obmen@sheree.dp.ua' 
IF @Testing = 1 SET @recipients = 'rumyantsev@const.dp.ua'
...
BEGIN
    ...
    --Отправка email.
	BEGIN TRY  
        DECLARE @xml_prologue varchar(max) = 'SELECT N''<?xml version="1.0" encoding="UTF-8"?>'' + cast(val as nvarchar(max)) FROM Elit.dbo.temp_puzo'
		DECLARE @SQL_query nvarchar(max) = 'SET NOCOUNT ON; ' + @xml_prologue + '; SET NOCOUNT OFF;'
		
		EXEC msdb.dbo.sp_send_dbmail  
		 @profile_name = 'main'
		,@recipients = @recipients
		,@blind_copy_recipients= 'pashkovv@const.dp.ua;rumyantsev@const.dp.ua;reznichenko@const.dp.ua;heavy@heavy.dp.ua' 
		,@subject = @subject
		,@query = @SQL_query
