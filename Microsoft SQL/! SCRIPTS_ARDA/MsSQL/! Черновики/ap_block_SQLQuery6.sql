...
--[CHANGED] rkv0 '2021-08-25 15:12' поставил проверку на ISNULL, т.к. иногда приходит пустое письмо (надо выловить).  
--[ADDED] rkv0 '2021-09-02 11:23' добавил выход из процедуры, чтобы не отправлялось пустое письмо (данные пока выловить не получилось).
...
SELECT  
    ...
    CASE   
        ...
        THEN 'Джоб: ' + (SELECT TOP 1 [name] FROM msdb.dbo.sysjobs WHERE job_id = msdb.dbo.[GetJobIdFromProgramName] (m.program))   
