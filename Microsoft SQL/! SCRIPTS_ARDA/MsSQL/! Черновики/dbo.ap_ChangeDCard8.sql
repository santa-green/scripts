USE [ElitR]
GO
/****** Object:  StoredProcedure [dbo].[ap_ChangeDCard]    Script Date: 13.05.2021 19:03:56 ******/
SET ANSI_NULLS ON
GO
...
--#region CHANGELOG
--[FIXED] rkv0 '2021-05-13 19:05' заменил @DCardChId на @OldDCardChId.
--#endregion CHANGELOG
...
--#region Замена ДК
    ...
    BEGIN
      ...
      WHERE DCardChID = @OldDCardChId AND NOT EXISTS (SELECT * FROM z_DocDC WHERE DocCode = dc.DocCode AND ChID = dc.ChID AND DCardChID = @DCardChId)
...
--#region r_DCardKin
--[REMOVED] rkv0 '2021-05-13 19:07' такой таблицы r_DCardKin нет.
/*      INSERT r_DCardKin
