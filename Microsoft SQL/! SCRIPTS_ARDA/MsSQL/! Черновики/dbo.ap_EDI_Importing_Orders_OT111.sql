USE [Alef_Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EDI_Importing_Orders_OT]    Script Date: 31.03.2021 14:53:55 ******/
SET ANSI_NULLS ON
GO
...
--[ADDED] rkv0 '2021-02-10 11:54' добавил блокировку заказов для сети РС Точка по заявке Драгнева (#4188).
--[ADDED] '2021-03-31 14:54' rkv0 добавил блокировку заказов для сети Копейка (7160	Модерн-Ритейл Общество с ограниченной ответственностью).
...
0	Новый заказ;
1	Заливается в ОТ;
3	Блокировка менеджера (Заказ еще не обработан (не проверен, не перезалит в ОТ) менеджером.
4	Задублированный;
5	Заказ залит в ОТ;
...
ZEO_ORDER_STATUS = 3 --3	БЛОКИРОВКА МЕНЕДЖЕРА;
WHERE 1 = 1
    AND ZEO_ORDER_STATUS = 0
    AND (ZEO_ZEC_BASE in (SELECT gln FROM [s-sql-d4].[elit].dbo.at_gln WHERE retailersid = 16568) --retailersid = 16568 для сети РС Точка согласно реестра GLN.
        OR ZEO_ZEC_ADD in (SELECT gln FROM [s-sql-d4].[elit].dbo.at_gln WHERE retailersid = 16568))

--[ADDED] '2021-03-31 14:54' rkv0 добавил блокировку заказов для сети Копейка (только для 7160 Модерн-Ритейл Общество с ограниченной ответственностью).
UPDATE dbo.ALEF_EDI_ORDERS_2
SET
ZEO_ORDER_STATUS = 3 --3	БЛОКИРОВКА МЕНЕДЖЕРА;
WHERE 1 = 1
    AND ZEO_ORDER_STATUS = 0
    AND ZEO_ZEC_BASE = '9864232419938' --SELECT * FROM [s-sql-d4].[elit].dbo.at_gln WHERE gln = '9864232419938'
