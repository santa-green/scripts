...
Импорт из ElitR заказ внешний формирование
в Alef_Elit ALEF_EDI_ORDERS_2 и ALEF_EDI_ORDERS_2_POS
в том числе заказы Решетняк (Решетняк уже давно не работает)*/
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*CHANGELOG*/
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Pashkovv '2019-10-22 09:23:36.622' добавил предприятие 80
...
IF OBJECT_ID (N'tempdb..#tmp', N'U') IS NOT NULL DROP TABLE #tmp
	...
	case        
		...
		when CompID = 80 then 981  --Pashkovv '2019-10-22 09:23:36.622' добавил предприятие 80
	end BaseGLN, 
	StockID AddGLN, 
	srcposid PosID, 
	Prodid, 
	Qty 
	INTO #tmp
	FROM [S-SQL-D4].[ElitR].[dbo].[t_EOExp] h --[t_EOExp] Заказ внешний: Формирование: Заголовок
	inner join [S-SQL-D4].[ElitR].[dbo].[t_EOExpD] d on h.chid = d.chid and OurID in (6,7,9,12) and CodeID3 = 8 --t_EOExpD Заказ внешний: Формирование: Товар
	and CompID in (71,81,171,181,82,80) --Pashkovv '2019-10-22 09:23:36.622' добавил предприятие 80
	WHERE DocDate >= dateadd(dd,-10,cast(CURRENT_TIMESTAMP as date))
