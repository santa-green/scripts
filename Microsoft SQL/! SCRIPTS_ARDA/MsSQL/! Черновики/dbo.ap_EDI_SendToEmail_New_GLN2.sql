USE [Alef_Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EDI_SendToEmail_New_GLN]    Script Date: 29.03.2021 17:59:12 ******/
SET ANSI_NULLS ON
GO
...
ZEO_ORDER_ID, [номер заказа], [дата заказа], [дата доставки], GLN, compid, compshort, mail, SendMailStatus
*/
IF EXISTS 
(
	select * 
	from dbo.ALEF_EDI_ORDERS_2 m
	where 1 = 1--zeo_zec_add NOT IN (SELECT EGS_GLN_ID FROM ALEF_EDI_GLN_SETI) 
	AND NOT EXISTS (SELECT * FROM ALEF_EDI_GLN_OT s1 WHERE s1.ZEC_KOD_BASE = m.ZEO_ZEC_BASE AND s1.ZEC_KOD_ADD = m.ZEO_ZEC_ADD)
  ...
  BEGIN TRY 
	...
	SELECT * FROM [S-SQL-D4].Elit.dbo.at_EDI_Orders_Not_GLN
		...
		BEGIN
			...
			--обновить статус SendMailStatus = 1 чтоб больше не приходили письма о новых адресах GLN
