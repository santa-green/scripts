USE [Elit_test]
GO
/****** Object:  StoredProcedure [dbo].[ap_ELIT_VAT_Tax_Payers_CHECK]    Script Date: 16.08.2021 10:29:18 ******/
SET ANSI_NULLS ON
GO
...
BEGIN --блок проверок.
    ...
    SELECT rc.TaxPayer, rc.CompType, rc.Code, rc.TaxCode, rc.CompID, rc.CompName, rc.CompShort, r2.CompGrID2, r2.CompGrName2, t.kod_pdv, t.dat_reestr, t.dat_anul, t.[name] FROM [Elit_TEST].dbo.r_comps rc
    --[CHANGED] rkv0 '2020-09-09 14:59' с 2020г. (нововведение от ДФС-https://uteka.ua/publication/news-14-ezhednevnyj-buxgalterskij-obzor-39-individualnyj-nalogovyj-nomer-platelshhika-nds-u-flp) для свидетельств НДС используется 12 цифр.
    ...
    JOIN [Elit_TEST].dbo.r_CompGrs2 r2 ON r2.CompGrID2 = rc.CompGrID2
    WHERE rc.TaxPayer = 0
        AND rc.CompType = 1 
        AND EXISTS 
            (SELECT * FROM [Elit_TEST].dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE())
    ...
    ELSE rc.TaxCode END
    WHERE EXISTS (SELECT * FROM [Elit_TEST].dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE())
    AND (
