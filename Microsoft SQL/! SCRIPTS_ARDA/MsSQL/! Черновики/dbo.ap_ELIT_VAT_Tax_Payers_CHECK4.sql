USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_ELIT_VAT_Tax_Payers_CHECK]    Script Date: 18.08.2021 12:40:10 ******/
SET ANSI_NULLS ON
GO
...
-- [ADDED] '2021-03-10 15:55' rkv0 добавил индекс для поля kod_pdv.
--[CHANGED] rkv0 '2021-08-16 12:57' добавил проверку на правильно заполенное поле ИНН (##RE-10121##).
--[CHANGED] rkv0 '2021-08-18 12:40' добавил проверку на правильно заполенное поле ИНН (##RE-10208##).


----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/* [1] СКАЧИВАНИЕ РЕЕСТРА, ПРЕОБРАЗОВАНИЕ, ЭКСПОРТ В ТАБЛИЦУ: (full) pdv.csv -> pdv_utf16.csv, инсерт данных в tempdb..Vat_check_result*/
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT CURRENT_TIMESTAMP 'CHECKPOINT 1: СКАЧИВАНИЕ РЕЕСТРА, ПРЕОБРАЗОВАНИЕ, ЭКСПОРТ В ТАБЛИЦУ'
...
SELECT CURRENT_TIMESTAMP 'CHECKPOINT 2: Блок проверок'
...
BEGIN --блок проверок.
    ...
    --[CHANGED] rkv0 '2021-08-16 12:57' добавил проверку на правильно заполенное поле ИНН (##RE-10121##).
    --WHERE rc.TaxPayer = 0 --закладка "Общие данные": галочка "Плательщик НДС".
    WHERE (rc.TaxPayer = 0 OR LEN(rc.TaxCode) < 10) --закладка "Общие данные": галочка "Плательщик НДС".
        AND rc.CompType = 1 --закладка "Общие данные": "Тип предприятия".
        AND EXISTS 
            (SELECT TOP 1 1 FROM Elit.dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE()) --закладка "Договора": "действующий" договор, конечная дата > сегодня.
    ...
    --[CHANGED] rkv0 '2021-08-18 12:40' добавил проверку на правильно заполенное поле ИНН (##RE-10208##).
    --WHERE rc.TaxPayer = 0
    WHERE (rc.TaxPayer = 0 OR LEN(rc.TaxCode) < 12) --закладка "Общие данные": галочка "Плательщик НДС".
        AND rc.CompType = 0 
        AND EXISTS 
            (SELECT TOP 1 1 FROM Elit.dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE())
        AND t.dat_anul IS NULL
        -- [ADDED] rkv0 '2020-09-23 17:43' добавил ограничение по дате регистрации (выбираем только, те даты, которые наступят завтра).
        AND ISDATE(t.dat_reestr) = 1 
        AND DATEADD(DAY, 1, CAST(GETDATE() AS DATE)) >= cast(t.dat_reestr as date)
    ...
    ELSE rc.TaxCode END
    WHERE EXISTS (SELECT TOP 1 1 FROM Elit.dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE())
    AND (
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT CURRENT_TIMESTAMP 'CHECKPOINT 3: Блок отправки email'
...
/* [4] LOG UPDATE*/
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT CURRENT_TIMESTAMP 'CHECKPOINT 4: LOG UPDATE'
...
BEGIN --обновление лога.
    ...
    -- [ADDED] '2020-03-06 11:48' rkv0 добавил timestamp (для сравнения)
    ...
    IF EXISTS (SELECT TOP 1 1 FROM #except_vat_check)
    BEGIN
...
SELECT CURRENT_TIMESTAMP 'CHECKPOINT 5: THE END.'
