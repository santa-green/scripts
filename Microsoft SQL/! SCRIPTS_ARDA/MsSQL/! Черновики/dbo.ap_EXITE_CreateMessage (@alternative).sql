...
DECLARE @OutChID INT, @ErrMsg VARCHAR(250); exec [ap_EXITE_CreateMessage] @MsgType ='INVOICE', @DocCode = 11012, @ChID = 200485357, @OutChID = @OutChID OUTPUT, @ErrMsg = @ErrMsg OUTPUT; SELECT @OutChID, @ErrMsg
...
--TEST
    ...
    DECLARE @docsum_count tinyint = (SELECT count(Notes) FROM [S-PPC.CONST.ALEF.UA].[alef_elit].[dbo].[av_EDI_Recadv2Invoice_METRO] WITH(NOLOCK) WHERE Notes = @orderid)
    DECLARE @alternative int = (SELECT CASE WHEN @docsum_count >= 2 THEN 0 ELSE @base_sum END)
