USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_CreateMessage]    Script Date: 28.05.2021 16:09:18 ******/
SET ANSI_NULLS ON
GO
...
--[ADDED] добавил проверку на заполнение поля "Погрузка-Ожидается" в РН для предприятий: 7004, 7144, 7001, 7003, 7142.
...
IF NOT (
	...
	BEGIN
        SELECT 'Для данного предприятия для отправки документа ORDRSP/DESADV (подтверждение заказа/уведомление об отгрузке) поле "Погрузка/Ожидается" должно быть заполнено: дата (не меньше текущей) и время.'
        RETURN
    END;
END;
