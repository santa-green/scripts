USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_CreateMessage]    Script Date: 20.08.2021 17:07:25 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROC [dbo].[ap_EXITE_CreateMessage] @MsgType VARCHAR(20)
,                                         @DocCode INT
,                                         @ChID INT
,                                         @OutChID INT OUTPUT
...
DECLARE @OutChID INT, @ErrMsg VARCHAR(250); exec [ap_EXITE_CreateMessage] @MsgType ='ORDRSP', @DocCode = 11012, @ChID = 200504894, @OutChID = @OutChID OUTPUT, @ErrMsg = @ErrMsg OUTPUT; SELECT @OutChID, @ErrMsg
DECLARE @OutChID INT, @ErrMsg VARCHAR(250); exec [ap_EXITE_CreateMessage] @MsgType ='DECLAR', @DocCode = 11012, @ChID = 200533707, @OutChID = @OutChID OUTPUT, @ErrMsg = @ErrMsg OUTPUT; SELECT @OutChID, @ErrMsg
...
--[ADDED] rkv0 '2021-05-28 17:00' добавил проверку на заполнение поля "Погрузка-Ожидается" в РН для предприятий: 7004, 7144, 7001, 7003, 7142.
--[ADDED] rkv0 '2021-05-31 11:43' для METRO (7001, 7003) добавил возможность добавлять дату на 1 день раньше.
--[SECURITY] rkv0 '2021-06-02 16:35' добавил в исключение Галю.
...
BEGIN TRY
EXEC [dbo].[ap_EXITE_ExportDoc] @DocType = @DocType
,                               @DocCode = @DocCode
,                               @ChID    = @ChID
,                               @Out     = @X OUT
,                               @TOKEN   = @TOKEN OUT
...
OPTION (MAXRECURSION 0) --You can provide the MAXRECURSION hint with any value between 0 and 32,767 within your T-SQL query, with MAXRECURSION value equal to 0 means that no limit is applied to the recursion level.  This opens up the risk of an infinite loop with poorly written queries. 
RETURN
END CATCH
...
EXEC dbo.z_DocLookup 'OurID'
,                    @DocCode
,                    @ChID
,                    @OurID OUT
--#endregion Подготовка

--#region Проверка РН != RECADV при отправке INVOICE (сеть METRO)
--[ADDED] rkv0 '2020-11-05 15:54' добавил проверку на совпадение количества в РН и в RECADV для сети METRO. Если количества не сходятся, отправка документа СЧЕТ/INVOICE блокируется (выдаем ошибку).
IF @DocType = 'INVOICE'
BEGIN

	DECLARE @OrderID varchar(250) = (SELECT OrderID
	FROM t_Inv WITH(NOLOCK)
	WHERE ChID = @ChID)

	DECLARE @base_sum int = (SELECT SUM(d.Qty / ISNULL([dbo].[af_GetQtyInUM] (d.ProdID,'метро/един.'),1) )
	FROM t_Inv  m
	JOIN t_InvD d ON d.ChID = m.ChID
	WHERE m.OrderID = @OrderID)

	DECLARE @docsum_count tinyint = (SELECT count(Notes)
	FROM [S-PPC.CONST.ALEF.UA].[alef_elit].[dbo].[av_EDI_Recadv2Invoice_METRO] WITH(NOLOCK)
	WHERE Notes = @orderid)

	DECLARE @alternative int = (SELECT CASE WHEN @docsum_count >= 2 THEN 0
	                                                                ELSE @base_sum END)
	...
	END;
END;
--#endregion Проверка РН != RECADV при отправке INVOICE (сеть METRO)

--#region проверку на заполнение поля "Погрузка-Ожидается"

--[ADDED] rkv0 '2021-05-28 17:00' добавил проверку на заполнение поля "Погрузка-Ожидается" в РН для предприятий: 7004, 7144, 7001, 7003, 7142.
--[SECURITY] rkv0 '2021-06-02 16:35' добавил в исключение Галю.
--IF @DocType in ('ORDRSP', 'DESADV')
IF @DocType in ('ORDRSP', 'DESADV') and SUSER_NAME() NOT IN ('corp\rumyantsev', 'kgv5')
BEGIN
IF NOT (
		(SELECT CAST(ExpDate AS date)
		FROM at_t_InvLoad
        --[ADDED] rkv0 '2021-05-31 11:43' для METRO (7001, 7003) добавил возможность добавлять дату на 1 день раньше.
		--WHERE ChID = @ChID) >= (SELECT CONVERT(date, GETDATE(), 102))
		WHERE ChID = @ChID) >= (SELECT CASE
                                        WHEN (SELECT Compid FROM t_inv WHERE ChID = @ChID) in (7001, 7003) THEN DATEADD(day, -1, CONVERT(date, GETDATE(), 102))
                                        ELSE CONVERT(date, GETDATE(), 102)
                                        END
            )
		AND (SELECT CAST(ExpDate AS time)
		FROM at_t_InvLoad
		WHERE ChID = @ChID) > '00:00')
        AND EXISTS (SELECT TOP 1 1 FROM t_inv WHERE chid = @chid AND COMPID in (7004, 7144, 7001, 7003, 7142))
        --[FIXED] rkv0 '2021-06-01 18:18' добавил условие: если в таблице at_t_InvLoad...
        OR NOT EXISTS (SELECT TOP 1 1 FROM at_t_InvLoad m JOIN t_Inv d ON m.ChID = d.ChID WHERE m.ChID = @ChID)
            AND EXISTS (SELECT * FROM t_inv WHERE CompID in (7004, 7144, 7001, 7003, 7142) and ChID = @ChID)
	BEGIN
        SELECT 'Для данного предприятия для отправки документа ORDRSP/DESADV поле "Погрузка/Ожидается" должно быть заполнено: дата (не меньше текущей) и время. Для сети METRO допускается -1 день.'
        RETURN
    END;
END;
--#endregion проверку на заполнение поля "Погрузка-Ожидается"

--#region Формируем имя файла для DECLAR
IF @DocType = 'DECLAR'
BEGIN
	...
	SET @FName = (SELECT '3200' --код ДПИ получателя [4 символа, <C_REG>, <C_RAJ>].
	...
	+ RIGHT('00000' + CAST(m.TaxDocID AS VARCHAR) + CAST(ISNULL((SELECT COUNT(*)
	FROM dbo.z_DocLinks         zdlt WITH(NOLOCK)
	JOIN dbo.at_z_FilesExchange zfet WITH(NOLOCK) ON zfet.ChID = zdlt.ChildChID
	WHERE zdlt.ParentChID = m.ChID
		AND zdlt.ParentDocCode = 11012
		AND zdlt.ChildDocCode = 666029
		AND zfet.StateCode = 403),0) + 1 AS VARCHAR), 7) --(например: 0003039) порядковый номер документа, который может отправляться несколько раз в одном отчетном периоде [7 символов, <C_DOC_CNT>]
	...
	Where m.Chid = @ChID)
END;
--#endregion Формируем имя файла для DECLAR

--#region PACKAGE_METRO
...
ELSE IF @DocType = 'COMDOC_METRO'
	...
	ELSE IF @DocType = 'PACKAGE_METRO'
		...
		END;
--#endregion PACKAGE_METRO

--#region INSERT at_z_FilesExchange, UPDATE r_CompValues
BEGIN TRAN

EXEC dbo.z_NewChID 'at_z_FilesExchange'
,                  @CH OUT

EXEC dbo.z_NewDocID 666029
,                   'at_z_FilesExchange'
,                   @OurID
,                   @DID OUT
...
,      GETDATE()                                                          
,      @OurID                                                             
,      CASE WHEN (SELECT compid
	from t_inv
	WHERE ChID = @ChID) in (7001,7003)
	AND @DocType in ('COMDOC_METRO','DECLAR','PACKAGE_METRO') THEN 502
                                                              ELSE 402 END StateCode
,      @X --статус 502 назначаем только METRO: comdoc, declar.            

EXEC [dbo].[ap_LinkCreate] @PDocCode = @DocCode
,                          @PChID    = @ChID
,                          @СDocCode = 666029
,                          @СChID    = @CH
...
USING (SELECT c.CompID          
,             'EDIINTERCHANGEID' VarName
,             RIGHT(@TOKEN, 4)   VarValue
FROM dbo.r_Comps b
...
ON (
	...
	AND target.VarName = source.VarName)
WHEN MATCHED
THEN UPDATE
SET target.VarValue = source.VarValue
WHEN NOT MATCHED
...
SET @OutChID = @CH
RETURN
--#endregion INSERT at_z_FilesExchange, UPDATE r_CompValues
