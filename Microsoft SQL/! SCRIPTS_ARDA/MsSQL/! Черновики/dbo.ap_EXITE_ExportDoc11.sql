/****** Object:  StoredProcedure [dbo].[ap_EXITE_ExportDoc]    Script Date: 11.03.2021 15:56:44 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='DESADV', @DocCode=11012, @ChID=200476555, /*PO001TX0V6*/  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
...
BEGIN
	    ...
	    --Номер.
        DECLARE @query_number NVARCHAR(MAX), @OutByDynamicSQLRecadvNumber_Silpo VARCHAR(50)
        SET @query_number = 'SELECT @outs = (SELECT aerf.ID FROM [s-ppc.const.alef.ua].[Alef_Elit].[dbo].[at_EDI_reg_files] aerf WHERE Notes LIKE m.OrderID + ''%'' AND aerf.doctype = 5000 AND aerf.RetailersID = 14)'
					    + ' FROM dbo.t_Inv m WITH(NOLOCK) '
					    + ' WHERE m.ChID = ' + CAST(/*@ChID*/200445987 AS VARCHAR(10))
	    EXEC sp_executesql @query_number, N'@outs VARCHAR(50) OUT', @OutByDynamicSQLRecadvNumber_Silpo OUT
        SELECT @OutByDynamicSQLRecadvNumber_Silpo

        --Дата.
        DECLARE @query_date NVARCHAR(MAX), @OutByDynamicSQLRecadvDate_Silpo date
        SET @query_date = 'SELECT @outs = (SELECT aerf.InsertData FROM [s-ppc.const.alef.ua].[Alef_Elit].[dbo].[at_EDI_reg_files] aerf WHERE Notes LIKE m.OrderID + ''%'' AND aerf.doctype = 5000 AND aerf.RetailersID = 14)'
			        + ' FROM dbo.t_Inv m WITH(NOLOCK) '
			        + ' WHERE m.ChID = ' + CAST(/*@ChID*/200445987 AS VARCHAR(10))
...
--CONTRL - Початок документа
      ISNULL(@OutByDynamicSQLRecadvNumber_Silpo, '111') AS 'NUMBER' --Номер документу основи (повідомлення про прийом RECADV)
    , cast(GETDATE() as date) AS 'DATE' --Дата документа
    , ISNULL(@OutByDynamicSQLRecadvDate_Silpo, '20200712') AS 'RECADVDATE' --Дата документа RECADV
    ...
    , COALESCE(irx.XMLData.value('(./HEAD/RECIPIENT)[1]', 'VARCHAR(13)'), irx.XMLData.value('(./ORDER/HEAD/RECIPIENT)[1]', 'VARCHAR(13)')) AS 'RECIPIENT' --GLN одержувача документу (покупця)
