...
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='DESADV', @DocCode=11012, @ChID=200500384,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
...
BEGIN
	...
	, CAST(m.TaxDocDate AS DATE)                                                                     'DATE'
	, CAST(m.TaxDocDate AS DATE)                                                                     'DELIVERYDATE'
	--[IN PROGRESS...][ADDED] '2021-05-21 12:45' rkv0 добавил условие для сети Сильпо (вода 16-00, крепкая  15-00) ##RE-5650##.
    , CASE WHEN CAST(il.ExpDate AS TIME) <> '00:00:00' THEN LEFT(CAST(il.ExpDate AS TIME), 5) END    'DELIVERYTIME'
/*    , CASE 
           --WHEN m.Compid in (7142) AND 1 = 1 /*крепкая группа*/ THEN '16:00'
           --WHEN m.Compid in (7142) AND 2 = 2 /*вода*/ THEN '15:00'
           WHEN m.Compid in (7142) AND CAST(il.ExpDate AS TIME) <> '00:00:00'/*..temp..*/ AND 2 = 2 /*вода*/ THEN '15:00'
           WHEN CAST(il.ExpDate AS TIME) <> '00:00:00' AND m.CompID NOT IN (7142) THEN LEFT(CAST(il.ExpDate AS TIME), 5) 
        END    'DELIVERYTIME'*/
...
--#region COMDOC (Коммерческий документ: расходная накладная для сети Розетка)
-- [CHANGED] rkv0 '2020-10-21 19:05' изменил COMDOC на COMDOC_ROZETKA, т.к. эта расходная отправляется только на Розетку (также это будет использоваться для ограничения отправки расходных из инструмента F11 Exite).
...
END;
--#endregion COMDOC (Коммерческий документ: расходная накладная для сети Розетка)
--#region COMDOC (Коммерческий документ: расходная накладная для сети Велике Пузо)
-- [ADDED] rkv0 '2021-01-16 16:37' добавил для сети "Велике Пузо".
...
/*                        CASE WHEN RIGHT(SUBSTRING(CAST(
    ...
    FOR XML PATH ('ЕлектроннийДокумент'))
  END;
--#endregion COMDOC (Коммерческий документ: расходная накладная для сети Велике Пузо)
