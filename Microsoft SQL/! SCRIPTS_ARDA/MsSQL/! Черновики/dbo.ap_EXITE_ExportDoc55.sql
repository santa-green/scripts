USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_ExportDoc]    Script Date: 16.03.2021 12:23:44 ******/
SET ANSI_NULLS ON
GO
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
DECLARE @xml XML, @token INT; EXEC ap_EXITE_ExportDoc @DocType='COMDOC_METRO', @DocCode=11012, @ChID=200479794,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token 'Token', @xml 'XML';
...
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='ORDRSP', @DocCode=11012, @ChID=200446661,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='DESADV', @DocCode=11012, @ChID=200476555,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='DESADV', @DocCode=11012, @ChID=200478842,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    -- '2019-10-15 16:50' rkv0 меняю подстановку № уведомления-№ заказа на №уведомления-№расходной
      ...
      CAST(m.TaxDocDate AS DATE) 'DATE',
      CAST(m.TaxDocDate AS DATE) 'DELIVERYDATE',
      CASE WHEN CAST(il.ExpDate AS TIME) <> '00:00:00' THEN LEFT(CAST(il.ExpDate AS TIME), 5) END 'DELIVERYTIME', --ExpDate = expected date.
      CASE WHEN ir.OrderID LIKE '%[_]non-alc' OR ir.OrderID LIKE '%[_]alc' THEN REPLACE(REPLACE(ir.OrderID, '_non-alc', ''), '_alc', '') ELSE ir.OrderID END 'ORDERNUMBER',
      irx.XMLData.value('(./ORDER/DATE)[1]', 'VARCHAR(13)') AS 'ORDERDATE',
      ...
      CAST(il.BoxQty AS INT) 'TOTALPACKAGES', --количество ящиков.
      CAST(il.PalQty AS INT) 'TOTALPALLETS', --количество паллет.
      zc.ContrID 'CAMPAIGNNUMBER', --Номер договору на поставку
      ...
      CASE WHEN m.Compid in (7142) THEN 1 ELSE NULL END 'TRANSPORTQUANTITY', --(тег обязательный для Сильпо) Загальна кількість машин; кількість повідомлень про відвантаження в межах одного замовлення покупця(число >1 або =1).
...
-- [CHANGED] '2021-03-12 17:56' rkv0 для Сильпо захардкодил 1 (всегда будет 1 машины).
      ...
      CASE WHEN m.Compid in (7142) THEN 1 ELSE NULL END 'TRANSPORTID', --(тег обязательный только для Сильпо) Порядковий номер транспортного засобу/повідомленя про відвантаження (число < або = TRANSPORTQUANTITY й не може бути більше)
      ...
      @TOKEN 'EDIINTERCHANGEID',
        (
          SELECT 1 'HIERARCHICALID', --Номер ієрархії упаковки
          (
...
--            dd.PriceCC_nt 'PRICE', 
            ...
            FROM (SELECT MIN(SrcPosID) SrcPosID
				  ...
				  LEFT JOIN dbo.r_ProdEC irec WITH(NOLOCK) ON irec.ProdID = di.ProdID AND irec.CompID = @CompID --Заказ внутренний: Формирование: Товар (! если на момент формирования DESADV будет отвязан код товара, то эта позиция не попадет в уведомление).
            ...
            WHERE -- Для некоторых предприятий не показывать нули
				  (m.CompID = 7030 AND dd.Qty IS NOT NULL) OR m.CompID != 7030 --7030	Эко Общество с ограниченной ответственностью  ( магазин Экомаркет)
    ...
    LEFT JOIN dbo.at_r_Drivers rd WITH(NOLOCK) ON rd.DriverID = m.DriverID AND rd.DriverId != 0 --at_r_Drivers	Справочник водителей
    LEFT JOIN dbo.r_Codes4 [rc4] WITH(NOLOCK) ON [rc4].CodeID4 = m.CodeID4 AND [rc4].CodeID4 != 0 --r_Codes4	Справочник признаков 4 (rc4 =>-https://docs.microsoft.com/en-us/sql/relational-databases/security/encryption/choose-an-encryption-algorithm?view=sql-server-ver15)
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    SELECT
      ...
      --[ADDED] rkv0 '2020-12-03 18:08' добавил теги: TIME, CURRENCY, INFO, CAMPAIGNNUMBER (обязательные для Сильпо)
	  LEFT(CONVERT(time, getdate()),5) AS 'TIME', --Время отправки документа
      'UAH' AS 'CURRENCY', --Валюта
      CASE WHEN m.Compid in (7142) THEN 'Поставка для сети Сильпо' ELSE NULL END AS 'INFO', --Вільний текст
      zc.ContrID 'CAMPAIGNNUMBER', --Номер договору на поставку
    ...
    JOIN dbo.at_t_IORecX irx WITH(NOLOCK) ON irx.ChID = @OrdChID
    --[ADDED] rkv0 '2020-12-03 18:08' джойним еще 2 таблицы для добавления тега CAMPAIGNNUMBER (обязательный тег для Сильпо).
    LEFT JOIN dbo.z_DocLinks zdl WITH(NOLOCK) ON zdl.ChildChID = m.ChID AND zdl.ChildDocCode = 11012 AND zdl.ParentDocCode = 666028 --ParentDocCode	Код гл. документа
    LEFT JOIN dbo.at_z_Contracts zc WITH(NOLOCK) ON zc.ChID = zdl.ParentChID --at_z_Contracts	Договор универсальный
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*COMDOC (Коммерческий документ: расходная накладная для сети Розетка)*/
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- [CHANGED] rkv0 '2020-10-21 19:05' изменил COMDOC на COMDOC_ROZETKA, т.к. эта расходная отправляется только на Розетку (также это будет использоваться для ограничения отправки расходных из инструмента F11 Exite).
...
-- [ADDED] rkv0 '2021-01-16 16:37' добавил для сети "Велике Пузо".
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*COMDOC (Коммерческий документ: расходная накладная для сети Велике Пузо)*/
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- [ADDED] rkv0 '2020-12-04 13:37' добавил новый документ IFTMIN (Инструкция по транспортировке) для сети Сильпо (Фоззи).
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*IFTMIN (Инструкция по транспортировке)*/
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
--DOCUMENT - Дані про документи (початок блоку)
    ...
    , (SELECT 
              'ON' AS 'DOCTYPE' --Допустиме значення «ON» - IFTMIN робиться на базі замовлення
...
--HEAD Початок основного блоку
    , (SELECT 
          COALESCE(irx.XMLData.value('(./HEAD/RECIPIENT)[1]', 'VARCHAR(13)'), irx.XMLData.value('(./ORDER/HEAD/RECIPIENT)[1]', 'VARCHAR(13)')) AS 'CONSIGNOR' --GLN вантажовідправника (постачальника)
        ...
        , COALESCE(irx.XMLData.value('(./HEAD/SENDER)[1]', 'VARCHAR(13)'), irx.XMLData.value('(./ORDER/HEAD/SENDER)[1]', 'VARCHAR(13)')) AS 'RECIPIENT' --GLN одержувача повідомлення
            ...
            , (SELECT 
                 '1' /*пока хардкодим, больше одного пока не встречалось*/ AS 'POSITIONNUMBER' --Номер позиції
                ...
                ,ISNULL((SELECT PalType FROM at_t_InvLoad WHERE ChID = @ChID), '200') AS 'PACKAGETYPE' --Тип упаковки «(Тип палет: 09 – зворотний піддон; 200 – палета ISO 0 – 1/2 EURO Pallet; 201 – палета ISO 1 – 1/1 EURO Pallet)»
                --,'1' /*пока хардкодим, нужны допполя в РН*/ AS 'PACKAGEQUANTITY' --Кількість упаковок (=фактическое количество паллет).
                ,(SELECT PalQty FROM at_t_InvLoad WHERE ChID = @ChID) AS 'PACKAGEQUANTITY' --Кількість упаковок (=фактическое количество паллет).
                --,'6' /*пока хардкодим, нужны допполя в РН*/ AS 'PACKAGEWIGHT' --Вага
                ,ISNULL((SELECT CarPayload FROM at_t_InvLoad WHERE ChID = @ChID), 20) AS 'PACKAGEWIGHT' --Вага
                --,'16' /*пока хардкодим, нужны допполя в РН*/ AS 'MAXPACKAGEQUANTITY' --Максимальна кількість упаковок
                ,ISNULL((SELECT PalQtyMAX FROM at_t_InvLoad WHERE ChID = @ChID), 32) AS 'MAXPACKAGEQUANTITY' --Максимальна кількість упаковок
...
-- [ADDED] rkv0 '2020-12-07 18:48' добавил новый документ CONTRL (Отчет об отгрузке) для сети Сильпо (Фоззи).
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*CONTRL (Отчет об отгрузке)*/
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

--Спецификация здесь:-https://wiki.edi-n.com/uk/latest/XML/Fozzy_XML-structure.html#contrl
/*
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='CONTRL', @DocCode=11012, @ChID=200476555,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
*/
...
BEGIN
    ...
    BEGIN
        ...
        SET @query_number_recadv = 'SELECT @outs = (
            ...
            WHERE 1 = 1 
            AND Notes = (SELECT OrderID FROM dbo.t_Inv m WITH(NOLOCK) WHERE chid = ' + CAST(@chid as varchar) + ')
            AND aerf.doctype = 5000 
            AND aerf.RetailersID = 14
            )'
	    EXEC sp_executesql @query_number_recadv, N'@outs VARCHAR(50) OUT', @OutByDynamicSQLRecadvNumber_Silpo OUT
    ...
    BEGIN    
        DECLARE @query_date NVARCHAR(MAX), @OutByDynamicSQLRecadvDate_Silpo date
        SET @query_date = 'SELECT @outs = (
            SELECT convert(date, InsertData - 1, 102) ''Дата RECADV'' FROM [s-ppc.const.alef.ua].[Alef_Elit].[dbo].[at_EDI_reg_files] aerf
            WHERE 1 = 1 
            AND Notes = (SELECT OrderID FROM dbo.t_Inv m WITH(NOLOCK) WHERE chid = ' + CAST(@chid as varchar) + ')
            AND aerf.doctype = 5000 
            AND aerf.RetailersID = 14
            )'
        EXEC sp_executesql @query_date, N'@outs date OUT', @OutByDynamicSQLRecadvDate_Silpo OUT
...
--DECLARE @xml XML, @token INT; EXEC ap_EXITE_ExportDoc @DocType='CONTRL', @DocCode=11012, @ChID=200445987,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token 'Token', @xml 'XML';
