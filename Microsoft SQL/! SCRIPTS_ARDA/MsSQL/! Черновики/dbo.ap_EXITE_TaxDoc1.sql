USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_TaxDoc]    Script Date: 20.11.2020 17:26:38 ******/
SET ANSI_NULLS ON
GO
...
--[FIXED] rkv0 '2020-11-13 10:47' для сети METRO делаю замену символа "+" на пустоту "".
  ...
  #-EndDebug*/
  /*
  DECLARE @Out XML; EXEC [dbo].[ap_EXITE_TaxDoc] @ChID = 200427900, @DocCode = 11012, @IsConsolidated = 0, @Out = @Out OUT; SELECT @Out 
  */
...
--[FIXED] rkv0 '2020-11-13 10:47' для сети METRO делаю замену символа "+" на пустоту "". ##RE-2297##
		...
		SELECT @Out = (select 'J1201010.xsd' AS '@xsi:noNamespaceSchemaLocation', @Out for xml path('DECLAR'));
	END 
