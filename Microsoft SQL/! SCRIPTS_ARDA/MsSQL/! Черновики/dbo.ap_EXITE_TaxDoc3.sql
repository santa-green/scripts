USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_TaxDoc]    Script Date: 17.03.2021 10:34:38 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  /*
  DECLARE @Out XML; EXEC [dbo].[ap_EXITE_TaxDoc] @ChID = 200475315, @DocCode = 11012, @IsConsolidated = 0, @Out = @Out OUT; SELECT @Out 
  */
  SET NOCOUNT ON
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  /*INFO*/
...
-- [CHANGED] '2021-03-17 10:36' rkv0 изменил версию с 11 на 12: <xs:element name="C_DOC_SUB" type="xs:string" fixed="010"/> (заявка #5763).
-- [CHANGED] '2021-03-17 10:35' rkv0 изменил версию xsd 'J1201011.xsd' на 'J1201012.xsd' (заявка #5763).
...
/*        'COMDOC:' + CAST(ISNULL((SELECT MAX([AEI_DOC_ID]) FROM [dbo].[az_EDI_Invoices_] WITH(NOLOCK) WHERE [AEI_INV_ID] = m.TaxDocID AND [AEI_INV_DATE] = m.TaxDocDate), m.TaxDocID) AS VARCHAR) + ';DATE:' + CONVERT(VARCHAR(10), m.TaxDocDate, 120)+';BY:' + irx.XMLData.value('(./ORDER/HEAD/SENDER)[1]', 'VARCHAR(13)') +';SU:' + irx.XMLData.value('(./ORDER/HEAD/SUPPLIER)[1]', 'VARCHAR(13)') 
		...
		SELECT @Out = (select 'J1201012.xsd' AS '@xsi:noNamespaceSchemaLocation', @Out for xml path('DECLAR')); --DECLAR Початок документу
	END 
