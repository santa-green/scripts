USE [ElitR]
GO
/****** Object:  StoredProcedure [dbo].[ap_Rkiper_Import_Sale]    Script Date: 18.05.2021 14:09:20 ******/
SET ANSI_NULLS ON
GO
...
SELECT 	
  ...
  /* 
  В отчете кипера "Расход блюд!":
  CURRENCIES00 - "Валюта", 
  CLASSIFICATORGROUPS0001 - "Отдел фискальной регистрации".
  */                                        
...
declare sale cursor for 
	SELECT DISTINCT docid , crid  FROM t_Sale_R
        --pvm0 01-12-2016 добавил в условие склад 1314 
        --[CHANGED] rkv0 '2021-05-18 10:32' добавил условие по кассе CRID.
		--WHERE  import = 0 and Docid not in (select Docid from t_Sale where OurID in (6,12) and StockID in (1314,1315,1202))
		WHERE  import = 0 and EXISTS (select top(1) 1 from t_Sale ts, t_Sale_R tr where ts.OurID in (6,12) and ts.StockID in (1314,1315,1202) and ts.DocID = tr.Docid and ts.CRID = tr.CRID)
		--and Docdate = @Idate
		--and Docid  in (129008) --****************************************************************************************
		--or Docid  in (231313) 
		ORDER BY docid 
...
BEGIN--1 (cursor sale)
	...
	set @i = @i + 1
		...
		declare saled cursor for 
			...
			select prodid , DocTime, KDSIDENT, TaxDocID, [ЧП или Нет ] as CHP from t_Sale_R where docid = @docid and CRID = @crid
		...
		END
			...
			insert t_SaleD 
                ...
                --[CHANGED] rkv0 '2021-05-18 10:32' добавил условие по кассе CRID.
				--where sr.docid = @docid and sr.prodid =@prodid and sr.DocTime = @DocTime and sr.KDSIDENT = @KDSIDENT
				where sr.docid = @docid and sr.CRID = @crid and sr.prodid =@prodid and sr.DocTime = @DocTime and sr.KDSIDENT = @KDSIDENT
		...
		from t_Sale_R where docid = @docid and CRID = @crid
