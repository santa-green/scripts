USE [ElitR]
GO
/****** Object:  StoredProcedure [dbo].[ap_Rkiper_Import_Sale]    Script Date: 08.02.2021 16:01:10 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	WHERE CHECKNUM  between  130178 and 1000000
    ...
    and CHECKNUM not in (SELECT Docid FROM #Imported)
    and pc.STATE = 6
...
SELECT 	
  ...
  CASE WHEN (CLASSIFICATORGROUPS0001.NAME = 'Кухня') THEN 12  ELSE 6 END OurID,
  CASE WHEN (PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150 ) THEN 1314 
       WHEN (PrintChecks00."CHECKNUM" BETWEEN 135000 and 499999) THEN 1315
       WHEN (PrintChecks00."CHECKNUM" BETWEEN 800153 and 1000000) THEN 1202
       END StockID,
  ...
  CASE WHEN (CLASSIFICATORGROUPS0001.NAME= 'Кухня' and PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150)  THEN 161 --разделение по кассам                                           
  ...
  EMPLOYEES00."NAME",
  isnull (( select  OperID from [S-VINTAGE].ElitRTS501.dbo.r_Opers where EmpID in (
...
JOIN [S-VINTAGE].[SQL_RK7].dbo.CurrLines CurrLines00
...
JOIN [S-VINTAGE].[SQL_RK7].dbo.PrintChecks PrintChecks00
...
JOIN [S-VINTAGE].[SQL_RK7].dbo.Orders Orders00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.EMPLOYEES EMPLOYEES00
  ON (EMPLOYEES00.SIFR = Orders00.MainWaiter)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.SaleObjects SaleObjects00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.SessionDishes SessionDishes00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.MENUITEMS MENUITEMS00
  ON (MENUITEMS00.SIFR = SessionDishes00.Sifr)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.DISHGROUPS DISHGROUPS0000
  ON (DISHGROUPS0000.CHILD = MENUITEMS00.SIFR) AND (DISHGROUPS0000.CLASSIFICATION = 512)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CLASSIFICATORGROUPS CLASSIFICATORGROUPS0000
  ON CLASSIFICATORGROUPS0000.SIFR * 256 + CLASSIFICATORGROUPS0000.NumInGroup = DISHGROUPS0000.PARENT  
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.DishDiscounts DishDiscounts00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.UNCHANGEABLEORDERTYPES UNCHANGEABLEORDERTYPES00
  ON (UNCHANGEABLEORDERTYPES00.SIFR = Orders00.UOT)
JOIN [S-VINTAGE].[SQL_RK7].dbo.GLOBALSHIFTS GLOBALSHIFTS00
  ON (GLOBALSHIFTS00.MidServer = Orders00.MidServer) AND (GLOBALSHIFTS00.ShiftNum = Orders00.iCommonShift)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.OrderSessions OrderSessions00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CASHES CASHES00
  ON (CASHES00.SIFR = PrintChecks00.iCloseStation)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CASHGROUPS CASHGROUPS00
  ON (CASHGROUPS00.SIFR = PayBindings.Midserver)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CURRENCYTYPES CURRENCYTYPES00
  ON (CURRENCYTYPES00.SIFR = CurrLines00.iHighLevelType)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CURRENCIES CURRENCIES00
  ON (CURRENCIES00.SIFR = CurrLines00.Sifr)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.Shifts Shifts00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.TABLES TABLES00
  ON (TABLES00.SIFR = Orders00.TableID)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.PaymentsExtra PaymentsExtra00
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.trk7EnumsValues trk7EnumsValues1E00
  ON (trk7EnumsValues1E00.EnumData = SaleObjects00.OBJKIND) AND (trk7EnumsValues1E00.EnumName = 'tSaleObjectKind')
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.RESTAURANTS RESTAURANTS00
  ON (RESTAURANTS00.SIFR = CASHGROUPS00.Restaurant)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.EMPLOYEES EMPLOYEES01
  ON (EMPLOYEES01.SIFR = SaleObjects00.iAuthor)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.SessionDishes SessionDishes01
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.MENUITEMS MENUITEMS01
  ON (MENUITEMS01.SIFR = SessionDishes01.Sifr)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.trk7EnumsValues trk7EnumsValues3600
  ON (trk7EnumsValues3600.EnumData = GLOBALSHIFTS00.STATUS) AND (trk7EnumsValues3600.EnumName = 'TRecordStatus')
  
  LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.DISHGROUPS DISHGROUPS0001
  ON (DISHGROUPS0001.CHILD = MENUITEMS00.SIFR) AND (DISHGROUPS0001.CLASSIFICATION = 768)

LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CATEGLIST CATEGLIST00
  ON (CATEGLIST00.SIFR = MENUITEMS00.PARENT)
  LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.CLASSIFICATORGROUPS CLASSIFICATORGROUPS0001
...
BEGIN--1 (cursor sale)
	...
	set @i = @i + 1
		...
		exec t_SaleAfterClose @chid, 0, 0, 0, '',0
--------------------------------------------------------------------------------
