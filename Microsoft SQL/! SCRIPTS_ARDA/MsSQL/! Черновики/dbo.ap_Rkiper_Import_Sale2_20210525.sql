USE [ElitR]
GO
/****** Object:  StoredProcedure [dbo].[ap_Rkiper_Import_Sale]    Script Date: 24.05.2021 15:35:17 ******/
SET ANSI_NULLS ON
GO
...
r_Opers Справочник ЭККА: операторы
r_Emps Справочник служащих
*/
--#endregion INFO
...
--[CHANGED] pvm0 15.11.2016 теперь курс доллара вычисляется автоматически /*zf_GetRateCC Возвращает курс валюты страны для указанного кода валюты*/
--[ADDED] gdn1- 01-12-2016 добавил разделение по складам и разделение по кассам на 1314 и 1315 
--[ADDED] pvm0 01-12-2016 добавил запись кода налога с поля SessionDishes00.iTaxDishType в поле t_Sale_R.TaxDocID
--[CHANGED] pvm0 02-12-2016 изменил импорт в продажу
--[ADDED] gdn1 9.02.2017  разделение на склад 1202 и разделение на кассы 104,106,108
...
--[CHANGED] rkv0 '2021-05-18 10:32' добавил условие по кассе CRID.
*/
--#endregion CHANGELOG
...
--#region выбираем новые чеки из кипера PrintChecks (с учетом фильтра)
	IF OBJECT_ID (N'tempdb..#CHECKNUM', N'U') IS NOT NULL
		DROP TABLE #CHECKNUM
	IF OBJECT_ID (N'tempdb..#Imported', N'U') IS NOT NULL
		DROP TABLE #Imported
	...
	WHERE 1 = 1
		and CHECKNUM between 130178 and 1000000 --диапазон чеков, который был выделен в ркипере при настройке его Димой.
		and CHECKNUM not in (462495,462496,810647,146819,148106,148772,149973,156400,157738,159011,159862,159949,456666,457035,457039,458019,458954,458955,460809,460811,462273,462281,462282,805260,805573,807308,807309) --Ганцев исключил проблемные чеки.
		and CHECKNUM not in (SELECT Docid
		FROM #Imported) --здесь исключаем уже залитые в t_Sale_R.
		and pc.[STATE] = 6

	SELECT *
	FROM #CHECKNUM
--#endregion выбираем новые чеки из кипера PrintChecks (с учетом фильтра)

--#region Добавление записей в t_Saler_R
INSERT t_Sale_R
SELECT 1                                                                         'ChID'
...
,
--[CHANGED] pvm0 15.11.2016 теперь курс доллара вычисляется автоматически /*zf_GetRateCC Возвращает курс валюты страны для указанного кода валюты*/
       dbo.zf_GetRateCC(840)                                                     'KursMC'
,
--Разделение по складам
       CASE WHEN (CLASSIFICATORGROUPS0001.[NAME] = 'Кухня') THEN 12
                                                            ELSE 6 END           'OurID'
, --12 - Куриленко Светлана Николаевна ФЛП, 6 - МаркетВин.
...
,      CASE WHEN (CURRENCYTYPES00."NAME" = 'Наличные') THEN 89
                                                       ELSE 27 END               'CodeID3'
,      0                                                                         'CodeID4'
,      0                                                                         'CodeID5'
,      0                                                                         'Discount'
...
--В отчете кипера "Расход блюд!": CURRENCIES00 - "Валюта", CLASSIFICATORGROUPS0001 - "Отдел фискальной регистрации".
       CASE WHEN (
		CLASSIFICATORGROUPS0001.[NAME] = 'Кухня'
		and PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150)  THEN 161
            WHEN (
		CLASSIFICATORGROUPS0001.[NAME]= 'Кухня'
		and PrintChecks00."CHECKNUM" BETWEEN 135000 and 499999)  THEN 160
            WHEN (
		CLASSIFICATORGROUPS0001.[NAME]= 'Кухня'
		and PrintChecks00."CHECKNUM" BETWEEN 800153 and 1000000) THEN 108
            WHEN (
		CLASSIFICATORGROUPS0001.[NAME]= 'Бар'
		AND CURRENCIES00."NAME" ='ЧП'
		and PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150)  THEN 107
--WHEN(CLASSIFICATORGROUPS0001.[NAME]= 'Бар.'AND CURRENCIES00."NAME" ='ЧП' and PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150 ) THEN 107
		...
		CLASSIFICATORGROUPS0001.[NAME]= 'Бар'
		AND CURRENCIES00."NAME" ='ЧП'
		and PrintChecks00."CHECKNUM" BETWEEN 135000 and 499999)  THEN 109
            WHEN (
		CLASSIFICATORGROUPS0001.[NAME]= 'Бар'
		AND CURRENCIES00."NAME" ='ЧП'
		and PrintChecks00."CHECKNUM" BETWEEN 800153 and 1000000) THEN 106
            WHEN (
		CURRENCIES00."NAME" = 'Мария'
		AND CLASSIFICATORGROUPS0001.[NAME] = 'Бар.'
		and PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150)  THEN 154
            WHEN (
		CURRENCIES00."NAME" = 'Мария'
		AND CLASSIFICATORGROUPS0001.[NAME] = 'Бар.'
		and PrintChecks00."CHECKNUM" BETWEEN 135000 and 499999)  THEN 153
            WHEN (
		CURRENCIES00."NAME" = 'Мария'
		AND CLASSIFICATORGROUPS0001.[NAME] = 'Бар.'
		and PrintChecks00."CHECKNUM" BETWEEN 800153 and 1000000) THEN 104
            WHEN (
		CURRENCIES00."NAME" = 'VISA'
		AND CLASSIFICATORGROUPS0001.[NAME] = 'Бар'
		and PrintChecks00."CHECKNUM" BETWEEN 450000 and 800150)  THEN 154
            WHEN (
		CURRENCIES00."NAME" = 'VISA'
		...
		CURRENCIES00."NAME" = 'Мария'
		AND CLASSIFICATORGROUPS0001.[NAME] ='Бар'
		and PrintChecks00."CHECKNUM" BETWEEN 800153 and 1000000) THEN 106 END    'CRID'
,
--CASE WHEN (CLASSIFICATORGROUPS0001.NAME= 'Кухня') THEN 160 WHEN(CLASSIFICATORGROUPS0001.NAME= 'Бар'AND CURRENCIES00."NAME" ='ЧП') THEN 109 ELSE 154 END CRID,
       EMPLOYEES00."NAME"                                                    
,      ISNULL (( select OperID
from [S-VINTAGE].ElitRTS501.dbo.r_Opers
where EmpID in ( 
	select EmpID
	from [S-VINTAGE].ElitRTS501.dbo.r_Emps
	where EmpName = EMPLOYEES00."NAME")),325)                                    'OperID'
,      ''                                                                        'CreditID'
,      OrderSessions00."PRINTAT"                                                 'DocCreateTime'
,      SessionDishes00.iTaxDishType                                              'TaxDocID'
, --pvm0 01-12-2016 добавил запись кода налога с поля SessionDishes00.iTaxDishType в поле TaxDocID
       '19000101'                                                                'TaxDocDate'
...
,      0                                                                         'TPurSumCC_nt'
,      0                                                                         'TPurTaxSum'
,      PrintChecks00.PAYFISCALSUM                                                'TPurSumCC_wt'
,      PrintChecks00."CLOSEDATETIME"                                             'DocTime'
,      0                                                                         'DepID'
,      0                                                                         'ClientID'
,      0                                                                         'InDocID'
,      ''                                                                        'ExpTime'
...
,      ''                                                                        'DeclDate'
,      0                                                                         'BLineID'
,      PrintChecks00.PAYFISCALSUM                                                'TRealSum'
,      0                                                                         'TLevySum'
,      0                                                                         'RemSchID'
...
,      GLOBALSHIFTS00."SHIFTNUM"                                                 'SHIFTNUM'
,      CASHES00."NAME"                                                           'CLOSESTATION'
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.SaleObjects            SaleObjects00 --SaleObjects                      
	ON (SaleObjects00.Visit = PayBindings.Visit)
		AND (SaleObjects00.MidServer = PayBindings.MidServer)
		AND (SaleObjects00.DishUNI = PayBindings.DishUNI)
		AND (SaleObjects00.ChargeUNI = PayBindings.ChargeUNI)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.SessionDishes          SessionDishes00 --SessionDishes                  
	ON (SessionDishes00.Visit = SaleObjects00.Visit)
		AND (SessionDishes00.MidServer = SaleObjects00.MidServer)
		AND (SessionDishes00.UNI = SaleObjects00.DishUNI)
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.DISHGROUPS             DISHGROUPS0000 --DISHGROUPS                      
	ON (DISHGROUPS0000.CHILD = MENUITEMS00.SIFR)
		AND (DISHGROUPS0000.CLASSIFICATION = 512)
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.DishDiscounts          DishDiscounts00 --DishDiscounts                  
	ON (DishDiscounts00.Visit = SaleObjects00.Visit)
		AND (DishDiscounts00.MidServer = SaleObjects00.MidServer)
		AND (DishDiscounts00.UNI = SaleObjects00.ChargeUNI)
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.OrderSessions          OrderSessions00 --OrderSessions                  
	ON (OrderSessions00.Visit = SaleObjects00.Visit)
		AND (OrderSessions00.MidServer = SaleObjects00.MidServer)
		AND (OrderSessions00.UNI = SaleObjects00.SessionUNI)
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.Shifts                 Shifts00 --Shifts (Смены: открытие, закрытие..)  
	ON (Shifts00.MidServer = PrintChecks00.MidServer)
		AND (Shifts00.iStation = PrintChecks00.iCloseStation)
		AND (Shifts00.ShiftNum = PrintChecks00.iShift)
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.PaymentsExtra          PaymentsExtra00 --PaymentsExtra                  
	ON (PaymentsExtra00.Visit = CurrLines00.Visit)
		AND (PaymentsExtra00.MidServer = CurrLines00.MidServer)
		AND (PaymentsExtra00.PayUNI = CurrLines00.PayUNIForOwnerInfo)
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.trk7EnumsValues        trk7EnumsValues1E00 --trk7EnumsValues            
	ON (trk7EnumsValues1E00.EnumData = SaleObjects00.OBJKIND)
		AND (trk7EnumsValues1E00.EnumName = 'tSaleObjectKind')
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.SessionDishes          SessionDishes01 --SessionDishes                  
	ON (SessionDishes01.Visit = SessionDishes00.Visit)
		AND (SessionDishes01.Midserver = SessionDishes00.Midserver)
		AND (SessionDishes01.UNI = SessionDishes00.ComboDishUNI)
...
LEFT JOIN [S-VINTAGE].[SQL_RK7].dbo.DISHGROUPS             DISHGROUPS0001 --DISHGROUPS                      
	ON (DISHGROUPS0001.CHILD = MENUITEMS00.SIFR)
		AND (DISHGROUPS0001.CLASSIFICATION = 768)
...
WHERE GLOBALSHIFTS00.[STATUS] = 3
	and PRINTAT < @ImportDate
	and CURRENCYTYPES00."NAME" <> 'Неплательщики'
	and CHECKNUM in (select CHECKNUM
	from #CHECKNUM)
order by CHECKNUM
...
--#endregion архив (фильтр)

--#endregion Добавление записей в t_Saler_R
  
--#region Курсор импорта в продажу
declare @docid     int
,       @chid      int
,       @SrcPosID  int
,       @prodid    int
...
,               crid
FROM t_Sale_R
--[ADDED] pvm0 01-12-2016 добавил в условие склад 1314
--[CHANGED] rkv0 '2021-05-18 10:32' добавил условие по кассе CRID.
--WHERE  import = 0 and Docid not in (select Docid from t_Sale where OurID in (6,12) and StockID in (1314,1315,1202))
WHERE import = 0
	and EXISTS (select top(1) 1
	from t_Sale   ts
	,    t_Sale_R tr
	where ts.OurID in (6,12)
		and ts.StockID in (1314,1315,1202)
		and ts.DocID = tr.Docid
		and ts.CRID = tr.CRID)
ORDER BY docid
...
--#region Создать ChID новый код регистрации для таблицы
	IF @docid <> @docid_old
	BEGIN
		SELECT @ChID = (ISNULL(MAX(ChID), @ChIDStart - 1) + 1)
		FROM t_Sale WITH(XLOCK, HOLDLOCK)
		WHERE ChID BETWEEN @ChIDStart AND @ChIDEnd
	...
	END
--#endregion Создать ChID новый код регистрации для таблицы

--#region Заголовки чеков - insert t_sale
	insert t_sale
	...
	,            (SELECT TOP 1 ChID
	FROM r_DCards rd
	WHERE rd.DCardID = sr.DCardID)
	from t_Sale_R sr
	where sr.docid = @docid
		and sr.import = 0
--#endregion Заголовки чеков - insert t_sale

	--#region Продажа товара (вложенный курсор)
	...
	BEGIN--3
		...
		BEGIN
			...
			select top 1 @chid                                                                             as chid
			,            @SrcPosID                                                                         as SrcPosID
			,            sr.ProdID                                                                        
			,            0                                                                                 as PPID
			,            r.UM                                                                             
			,            sr.Qty                                                                           
			...
			,            (select top 1 PLID
			from r_Stocks
			where StockID = sr.StockID)                                                                    as PLID
			, -- Выбор прайса по складу
			             Discount                                                                         
			,            0                                                                                 as DepID
			,            1                                                                                 as IsFiscal
			,            0                                                                                 as SubStockID
			,            1                                                                                 as OutQty
			,            EmpID                                                                            
			,            DocTime                                                                           as CreateTime
			,            @DocTime                                                                          as ModifyTime
			,            0                                                                                 as TaxTypeID
			,            (sr.SumCC_wt/sr.Qty)                                                              as RealPrice
			,            sr.SumCC_wt                                                                       as RealSum
			from         t_Sale_R sr
			join         r_Prods  r  on sr.prodid = r.ProdID
			join         r_ProdMP rm on r.ProdID = rm.ProdID
					and rm.PLID = (select top 1 PLID
					from r_Stocks   
					where StockID = sr.StockID)-- Выбор прайса по складу
			join         r_ProdMQ rq on r.ProdID = rq.ProdID
					and r.UM = rq.UM
			where sr.docid = @docid
				and sr.prodid =@prodid
				and sr.DocTime = @DocTime
				and sr.KDSIDENT = @KDSIDENT
		END
		--#endregion Отладка
	...
	close saled
	deallocate saled
	--#endregion Продажа товара (вложенный курсор)

--#region update t_Sale_R (импорт завершен)
	update t_Sale_R
	set import = 1
	,   chid =   @chid
	--[CHANGED] rkv0 '2021-05-18 10:32' добавил условие по кассе CRID.
	...
	where docid = @docid
		and CRID = @crid
--#endregion update t_Sale_R (импорт завершен)

--#region Оплаты - insert t_SalePays
	insert t_SalePays
	select distinct @chid                                              ChID
	,               1                                               as SrcPosID
	,               case when (PayFormCode = 'Наличные') then 1
	                                                     else 2 end as PayFormCode
	,               SUM(SumCC_wt)                                  
	,               ''                                              as Notes
	,               0                                               as POSPayID
	,               ''                                              as POSPayDocID
	...
	group by ChID
	,        PayFormCode
--#endregion Оплаты - insert t_SalePays

--#region Процедура после закрытия чека
	EXEC dbo.t_SaleAfterClose @chid
	,                         0
	,                         0
	,                         0
	,                         ''
	,                         0
--#endregion Процедура после закрытия чека

--#region Установка признаков документа
    --[ADDED] moa0 25.06.2019 15:09 добавил установку признака 3.
    --В теории можно сделать процедуру, которая будет устанавливать признаки в документе, а потом вызывать ее в нужно месте (передавать туда ChID продажи).
	IF (SELECT COUNT(*)
		...
		,   m.CodeID3 =  CASE p.PayFormCode WHEN 1 THEN (CASE WHEN m.StockID IN (1201) THEN 81
			                                                  WHEN m.StockID IN (1202,1315,1314)
				and m.CRID in ( 104,153,154,159)                                       THEN 81
			                                                  WHEN m.StockID IN (1202,1315,1314)
				and m.CRID in ( 105,106,108,109,160,161,107)                           THEN 89
	...
	ELSE
		...
		FROM t_Sale m WITH(NOLOCK)
		WHERE m.ChID = @ChID
--#endregion Установка признаков документа

--#region Продажа товара оператором UPDATE t_Sale
	UPDATE m
	SET m.TSumCC_nt =    ISNULL((SELECT SUM(SumCC_nt)
	FROM t_SaleD WITH(NOLOCK)
	WHERE ChID = m.ChID),0)
	,   m.TTaxSum =      ISNULL((SELECT SUM(TaxSum)
	FROM t_SaleD WITH(NOLOCK)
	WHERE ChID = m.ChID),0)
	,   m.TSumCC_wt =    ISNULL((SELECT SUM(SumCC_wt)
	FROM t_SaleD WITH(NOLOCK)
	WHERE ChID = m.ChID),0)
	,   m.TPurSumCC_wt = ISNULL((SELECT SUM(PurPriceCC_wt * Qty)
	FROM t_SaleD WITH(NOLOCK)
	WHERE ChID = m.ChID),0)
	,   m.TPurTaxSum =   ISNULL((SELECT SUM(PurTax * Qty)
	FROM t_SaleD WITH(NOLOCK)
	WHERE ChID = m.ChID),0)
	,   m.TPurSumCC_nt = ISNULL((SELECT SUM(PurPriceCC_nt * Qty)
	FROM t_SaleD WITH(NOLOCK)
	WHERE ChID = m.ChID),0)
	FROM t_Sale m
	WHERE m.ChID =@chid
--#endregion Продажа товара оператором UPDATE t_Sale

	--Запись старого @docid
	set @docid_old = @docid
	print '@docid_old = ' + str(@docid_old)

--#region Создание оплаты: INSERT t_MonRec (наличка)
	--EXEC dbo.apt_SaleInsertMonRec @ChID = @ChID
	IF 1 = 1
	BEGIN
		SELECT @MRChID = (ISNULL(MAX(ChID), @ChIDStart - 1) + 1)
		FROM t_MonRec WITH(XLOCK, HOLDLOCK)
		WHERE ChID BETWEEN @ChIDStart AND @ChIDEnd

		INSERT t_MonRec
		( ChID, OurID, AccountAC, DocDate, DocID, StockID, CompID, CompAccountAC, CurrID, KursMC, KursCC, SumAC, Subject, CodeID1, CodeID2, CodeID3, CodeID4, CodeID5, EmpID )
		SELECT ROW_NUMBER() OVER (ORDER BY m.DocID, p.PayFormCode) - 1 + @MRChID ChID
		,      OurID                                                            
		,      '0'                                                               'AccountAC'
		,      DocDate                                                          
		,      DocID                                                            
		...
		,      KursMC                                                           
		,      1                                                                 'KursCC'
		,      SUM(p.SumCC_wt)                                                   'SumAC'
		,      REPLACE(p.Notes,'Сдача','')                                       'Subject'
		,      CodeID1                                                          
		,      CodeID2                                                          
		,      CodeID3                                                          
		,      CASE m.StockID WHEN 704 THEN 0
		                               ELSE m.CodeID4 END                        'CodeID4'
		,      CASE m.StockID WHEN 704 THEN 0
		                               ELSE m.CodeID5 END                        'CodeID5'
		,      EmpID                                                            
		FROM t_Sale     m WITH(NOLOCK)
		JOIN t_SalePays p WITH(NOLOCK) ON p.ChID = m.ChID
		WHERE m.ChID = @ChID
			AND p.PayFormCode IN (1,5)
		GROUP BY OurID
		,        DocDate
		,        DocID
		,        StockID
	...
	END;
--#endregion Создание оплаты: INSERT t_MonRec (наличка)

--#region Установка статуса документа: UPDATE t_Sale
	UPDATE t_Sale
	SET StateCode = dbo.zf_Var('t_ChequeStateCode') --zf_Var Возвращает значение переменной; t_ChequeStateCode	Статус документа после печати на ЭККА.
	WHERE ChID = @ChID
--#endregion Установка статуса документа: UPDATE t_Sale
...
CLOSE sale
DEALLOCATE sale
--#endregion Курсор импорта в продажу
...
--проверка синхронизации ркипера (t_sale_r) и базы (t_sale)
SELECT * FROM (
SELECT * 
