USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_SendEmailEDI]    Script Date: 15.04.2021 15:53:36 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT ChID
	FROM [S-PPC.CONST.ALEF.UA].Alef_Elit.dbo.at_EDI_reg_files
	WHERE DocType = 70121 AND [Status] BETWEEN 9 AND 16 AND InsertData < (DATEADD(DAY, @delay, GETDATE()))
	...
	SELECT ChID
	FROM [S-PPC.CONST.ALEF.UA].Alef_Elit.dbo.at_EDI_reg_files
	WHERE DocType = 70121 AND ([Status] = 18 OR [Status] = 12)
		  AND ( (SELECT DATEDIFF(d, LastUpdateData, GETDATE())) >= 7) --Time at days, when 012 comdoc will have second chanse.
