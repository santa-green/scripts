USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_SendEmailEDI]    Script Date: 30.11.2020 17:07:17 ******/
SET ANSI_NULLS ON
GO
...
-- [ADDED] rkv0 '2020-11-30 17:07' добавляю данные по позициям и бутылкам; сеть Розетка (заявка #1876).
                    ...
                    ,(SELECT SUM(REP_ACCEPT_QTY) FROM [S-PPC.CONST.ALEF.UA].[alef_elit].dbo.ALEF_EDI_RECADV m
                        JOIN [S-PPC.CONST.ALEF.UA].[alef_elit].dbo.ALEF_EDI_RECADV_POS d ON m.REC_INV_ID = d.REP_INV_ID and m.REC_INV_DATE = d.REP_INV_DATE
                        WHERE m.REC_ORD_ID = @orderid) 'td', ' ' --Принято бутылок, шт. (recadv)
            ...
            SET @body = '<html> 
                ...
                <table border = "3" bordercolor=" #FF5733 "> 
                    <tr>
                        ...
                        <th><font color="#5e9572"> Отправлено бутылок (recadv), шт. </font></th>
                        <th><font color="#5e9572"> Отправлено позиций (РН), шт. </font></th>
                        <th><font color="#C70039"> Принято бутылок, шт. (recadv) </font></th>
                        <th><font color="#C70039"> Принято позиций (recadv) </font></th>
                    </tr>'
