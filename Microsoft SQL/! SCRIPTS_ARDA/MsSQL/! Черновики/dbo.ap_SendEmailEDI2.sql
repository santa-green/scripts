...
*/			
			...
			BEGIN TRY 
			  SET @subject = 'ДФС отклонила налоговую ' + CAST( (SELECT ID FROM [S-PPC.CONST.ALEF.UA].[Alef_Elit].[dbo].[at_EDI_reg_files] WHERE ChID = @ChID) AS VARCHAR)
			  SET @msg = '<p>' + (SELECT TOP 1 [FileName]
			                          FROM [S-PPC.CONST.ALEF.UA].Alef_Elit.dbo.at_EDI_reg_files reg
			                            	WHERE ChID = @ChID
			                          ORDER BY ChID DESC) + '</p>'
                        + '<p style="font-size: 12;color:gray"><i>Отправлено [S-PPC] JOB Workflow шаг 3 (Import_RPL / Import_Rpl_Email_reg_use.ps1)</i></p>'
			  --SET @sql = 
