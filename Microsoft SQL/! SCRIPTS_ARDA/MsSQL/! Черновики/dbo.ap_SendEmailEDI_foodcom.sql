USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_SendEmailEDI]    Script Date: 23.04.2021 11:56:44 ******/
SET ANSI_NULLS ON
GO
...
*/
    ...
    SELECT ChID FROM [S-PPC.CONST.ALEF.UA].[alef_elit].dbo.[at_EDI_reg_files] WHERE doctype = 70121 and [Status] = 10 and InsertData >= '20210101';
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            SELECT ID 'Номер возвратной накладной', CONVERT(varchar, InsertData, 104) 'Дата', DocSum 'Итого сумма с НДС, грн', 'https://edo-v2.edi-n.com/app/#/service/edi/chain/list/inbox/0' 'Сайт EDIN', 'Сеть: Велика Кишеня, #Накладная на возврат, Номер: ' + ID + ',' 'Поиск через фильтр в EDIN' INTO #html FROM [S-PPC.CONST.ALEF.UA].[alef_elit].dbo.[at_EDI_reg_files] WHERE doctype = 70121 and ChID = @ChID;
