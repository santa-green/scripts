USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_SendEmail_do_Blocked]    Script Date: 04.03.2021 18:23:53 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
 ...
 INTO ##GlobalTempTable_blocked
  ...
  WHERE ( (spid IN (SELECT blocked FROM sys.sysprocesses WHERE blocked <> 0) AND blocked = 0)  or (blocked <> 0) )
       ...
       SELECT distinct(cmd) FROM sys.sysprocesses
...
IF EXISTS (SELECT top 1 1 FROM ##GlobalTempTable_blocked)
BEGIN
