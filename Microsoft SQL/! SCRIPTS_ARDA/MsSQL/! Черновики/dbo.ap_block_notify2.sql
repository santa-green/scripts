USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_block_notify]    Script Date: 02.08.2021 13:41:43 ******/
SET ANSI_NULLS ON
GO
...
begin
    ...
    SET @recipients = (SELECT CASE WHEN @test = 1 THEN 'rumyantsev@const.dp.ua' ELSE 'support_arda@const.dp.ua' END)
    SELECT @recipients
	...
	BEGIN
		...
		SET @msg = '<p style="font-size: 12; color: gray"><i>[для администратора БД] Отправлено [S-PPC] JOB "ELIT Blocked" шаг 2 (Уведомление о долгой блокировке / [ap_block_notify] )</i></p>'
...
SELECT
    ...
    m.[status],
    m.loginame,
    CONVERT(CHAR(3), sp.blocked) 'Кем заблокирован',
    [database],
    ISNULL(gui.EmpID, 9999)           'Код сотрудника',
    ISNULL(gui.EmpName, '[нет инфо]') 'ФИО'
INTO #html
