...
ALTER PROCEDURE [dbo].[TEST_ap_block_notify] @test bit = NULL
...
begin
    ...
    /*
    --FOR TEST
    EXEC [dbo].[TEST_ap_block_notify] @test = 1
    */
    --DECLARE @test bit = 1
    DECLARE @recipients varchar(max)
    SET @recipients = (SELECT CASE WHEN @test = 1 THEN 'rumyantsev@const.dp.ua' ELSE 'rumyantsev@const.dp.ua' END)
    SELECT @recipients
    ...
    --WHERE (
	    ...
	    IF @test = 1 WAITFOR DELAY '00:00:01'
	...
	BEGIN
		DECLARE @subject varchar(max) = 'Блокировки (TESTING...)'
        ...
        '<p style="font-size: 12; color: gray"><i>[для администратора БД] Отправлено [S-PPC] JOB "ELIT Blocked" шаг 2 
            (Уведомление о долгой блокировке / [TEST_ap_block_notify] )</i></p>'
...
SELECT
    m.spid,
    m.[status],
    CASE WHEN m.loginame = 'CORP\cluster' THEN 'Системный пользователь CORP\cluster' ELSE m.loginame END 'Логин',
    CONVERT(CHAR(3), sp.blocked) 'Кем заблокирован: spid',
    CASE WHEN sp.blocked = 0 THEN '+' ELSE '-' END 'КТО БЛОКИРУЕТ',
    [database],
    m.host 'Имя компьютера',
    CASE 
        ...
        ELSE m.program END 'Приложение',
    ISNULL(gui.EmpID, 0000)           'Код сотрудника',
...
SELECT * FROM #html
    ...
    SET @copy_recipients = 'rumyantsev@const.dp.ua'
