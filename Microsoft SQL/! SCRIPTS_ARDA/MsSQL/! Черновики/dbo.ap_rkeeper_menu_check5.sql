USE [ElitR]
GO
/****** Object:  StoredProcedure [dbo].[ap_rkeeper_menu_check]    Script Date: 09.02.2021 17:27:00 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
    ...
    (SELECT COUNT(*) FROM #Baze WHERE ProdID IS NULL OR ProdID NOT IN (N'605229'/*Бизнес-ланч из 2-х блюд*/, N'605230'/*Бизнес-ланч из 3-х блюд*/)) 'Кол активных товаров в базе', 
...
IF EXISTS(SELECT TOP 1 1 FROM #Baze WHERE [Внешний код] is null AND ProdID NOT IN (SELECT [Внешний код] FROM #Menu_ALL)
    -- [CHANGED] rkv0 '2021-02-09 17:05' добавил в исключение инфо по комбо блюдам (Бизнес ланч 2 и 3).
     AND ProdID NOT IN (N'605229'/*Бизнес-ланч из 2-х блюд*/, N'605230'/*Бизнес-ланч из 3-х блюд*/)
) 
BEGIN
...
BEGIN
	...
	, [для акций], [для предчеков], Классификация, [Отдел ФР], [Сервис печать], Категория, Группа
	 FROM #Baze p WHERE p.ProdID NOT IN (SELECT [Внешний код] FROM #Menu)
