USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_s-ppc_linked_server_check]    Script Date: 29.06.2021 13:48:38 ******/
SET ANSI_NULLS ON
GO
...
SET NOCOUNT ON;
    ...
    DECLARE @srv SYSNAME = N's-ppc.const.alef.ua';
    --DECLARE @srv SYSNAME = N's-marketa'; --для теста.
    ...
    END TRY
    BEGIN CATCH
        SELECT @retval = ERROR_MESSAGE();
    END CATCH

    SELECT @retval 'RESULT'

    DECLARE @subject_1 nvarchar(max) = 's-ppc linked server отвалился! ' + @retval;
    DECLARE @subject_2 nvarchar(max) = 's-ppc linked server ожил!';
    ...
    */
    ...
    --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    /*Если s-ppc linked server НЕ коннектится: */
    --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    IF @retval <> '0'
        BEGIN
            ...
            IF (SELECT Net_Status FROM sppc_linked_server_log WHERE ID = (SELECT MAX(ID) FROM sppc_linked_server_log)) = 'False' 
                ...
                BEGIN
                    SELECT 's-ppc linked server НЕ коннектится..'
		            ...
		            ,@body = '<font size="5" color="red" face="Arial">s-ppc linked server отвалился!'
		            ,@subject = @subject_1
    ...
    --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   /*Если s-ppc linked server коннектится: */
    --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    IF @retval = '0'
        BEGIN
            --делаем запись об отправленном уведомлении в лог.
            ...
            IF (SELECT Net_Status FROM sppc_linked_server_log WHERE ID = (SELECT MAX(ID) FROM sppc_linked_server_log)) = 'True' 
                ...
                BEGIN
                    SELECT 's-ppc linked server ожил.'
                    ...
                    --,@recipients = 'support_arda@const.dp.ua;dirin@const.dp.ua;tumaliieva@const.dp.ua;bibik@const.dp.ua;shevchenkoao@const.dp.ua;martynyuk@const.dp.ua;yevtischenko@const.dp.ua;tancyura@const.dp.ua;klimenkoan@const.dp.ua'
		            ,@recipients = @recipients
		            ,@body = '<font size="5" color="green" face="Arial"> s-ppc linked server <b>ожил</b>'
		            ,@subject = @subject_2
		            ,@body_format = 'HTML'--'HTML'
                    ,@importance = 'LOW'
                END;
    ...
    --SELECT top 10000 * FROM sppc_linked_server_log WHERE net_status = 'false' --WITH(NOLOCK) ORDER BY Log_time DESC
