USE [msdb]
GO
/****** Object:  StoredProcedure [dbo].[ap_sendemail_failedjobs]    Script Date: 23.02.2021 17:08:42 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[ap_sendemail_failedjobs]
AS
BEGIN
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*DESCRIPTION*/
...
IF EXISTS(
    ...
    BEGIN
                ...
                --[ADDED] rkv0 '2021-02-23 15:41' добавил название джоба, шага и статуса.
                DECLARE @jobname varchar(max) = (SELECT DISTINCT(j.[name]) JobName FROM sysjobhistory h inner join sysjobs j ON j.job_id = h.job_id WHERE h.run_status IN (0,2,3) AND dbo.agent_datetime(h.run_date, h.run_time) > CAST(DATEADD(MINUTE, -15, SYSDATETIME()) AS datetime) /*and j.[name] not like '%_[dba]_%'*/)
                DECLARE @stepname varchar(max) = (SELECT DISTINCT(step_name) FROM sysjobhistory h inner join sysjobs j ON j.job_id = h.job_id WHERE step_id = (SELECT MAX(step_id) FROM sysjobhistory h inner join sysjobs j ON j.job_id = h.job_id WHERE h.run_status IN (0,2,3) AND dbo.agent_datetime(h.run_date, h.run_time) > CAST(DATEADD(MINUTE, -15, SYSDATETIME()) AS datetime)) AND h.run_status IN (0,2,3) AND dbo.agent_datetime(h.run_date, h.run_time) > CAST(DATEADD(MINUTE, -15, SYSDATETIME()) AS datetime))
                ...
                SET @subject = 'JOB FAILURE !! ' + @jobname + '. Status: ' + @jobstatus + '. ' + 'Step: ' + @stepid + ' - ' + @stepname;
