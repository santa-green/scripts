USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_ExportDoc]    Script Date: 03.12.2020 16:30:48 ******/
SET ANSI_NULLS ON
GO
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    -- '2019-10-15 16:50' rkv0 меняю подстановку № уведомления-№ заказа на №уведомления-№расходной
      ...
      CASE WHEN m.Compid in (7142) THEN ISNULL(CAST(il.CarQty AS INT), 1) ELSE rd.CarNo END 'TRANSPORTID', --(тег обязательный только для Сильпо) Порядковий номер транспортного засобу/повідомленя про відвантаження (число < або = TRANSPORTQUANTITY й не може бути більше)
...
/*
DECLARE @xml XML, @token INT;EXEC ap_EXITE_ExportDoc @DocType='IFTMIN', @DocCode=11012, @ChID=200445987,  @Out = @xml OUTPUT, @TOKEN = @token OUTPUT; SELECT @token, @xml;
*/
END;
