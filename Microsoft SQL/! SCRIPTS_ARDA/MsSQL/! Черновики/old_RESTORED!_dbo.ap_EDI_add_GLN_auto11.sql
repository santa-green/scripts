USE [Alef_Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EDI_add_GLN_auto]    Script Date: 20.08.2021 11:37:35 ******/
SET ANSI_NULLS ON
GO
...
IF 1 = 0
BEGIN
    SELECT * FROM ALEF_EDI_ORDERS_2 WHERE ZEO_ORDER_NUMBER = '4516559340' --базовый GLN можно взять здесь.
    SELECT * FROM ALEF_EDI_ORDERS_2 WHERE ZEO_ZEC_ADD = '9864232446125' --базовый GLN можно взять здесь.
    INSERT [S-SQL-D4].Elit.dbo.r_CompValues (CompID, VarName, VarValue) 
    --[added] rkv0 '20210915' VALUES (70997,'BASE_GLN','9864232445906') --(!) меняем параметры здесь.       
    VALUES (70997,'BASE_GLN','9864232445906') --(!) меняем параметры здесь.       
END;
...
BEGIN TRAN
    ...
    (
	    ...
	    ,      (SELECT Adress
	    FROM [S-SQL-D4].Elit.dbo.at_gln WITH (NOLOCK) --! этот адрес можно брать напрямую из r_compsAdd, но пока оставлю для сверки.
...
BEGIN
    ...
    BEGIN
        ...
        ,                            @subject            = @subject
        ,                            @body               = 'Это автоматическое уведомление [Джоб GLN_AUTO_ADD]'
        --,                            @body_format        = 'HTML'
        ,                            @append_query_error = 1
