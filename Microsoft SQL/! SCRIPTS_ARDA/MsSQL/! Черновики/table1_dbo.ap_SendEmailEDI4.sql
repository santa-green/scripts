USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_SendEmailEDI]    Script Date: 08.12.2020 15:40:55 ******/
SET ANSI_NULLS ON
GO
...
/*
update [S-PPC.CONST.ALEF.UA].Alef_Elit.dbo.at_EDI_reg_files set [status] = 3 WHERE chid = 72629 and doctype = 24000 and id = 'РОЗ01097216'
SELECT * FROM  [S-PPC.CONST.ALEF.UA].Alef_Elit.dbo.at_EDI_reg_files WHERE chid = 72629 and doctype = 24000 and id = 'РОЗ01097216'
...
-- [ADDED] rkv0 '2020-11-30 17:07' добавляю данные по позициям и бутылкам; сеть Розетка (заявка #1876).
            ...
            DECLARE @query varchar(max) = 'SET NOCOUNT ON; SELECT ''===>>>'' ''RECADV'', aer.REC_ORD_ID ''Номер заказа'', ti.CompID ''Предприятие'', aerp.REP_POS_KOD ''Код сети'', aerp.REP_ACCEPT_QTY ''Принято, бут.'', aerp.REP_DELIVER_QTY ''Отправлено, бут.'',
            ...
            WHERE ti.orderid = ' + '''' + cast(@orderid as varchar(250)) + '''' + '
