USE [Alef_Elit]
GO
/****** Object:  StoredProcedure [dbo].[ALEF_WEB_EDI_GET_ORDERS]    Script Date: 20.08.2021 15:31:00 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
--#region changelog
	--[CHANGED] rkv0 '2021-08-20 15:32' изменил на 3 месяца (одного часто не хватает).
--#endregion changelog
	...
	inner join
		...
		inner join dbo.ALEF_Split(@str,',') on Splitcolumn = EGS_GLN_SETI_ID) as T
		on EGS_GLN_ID = ZEO_ZEC_BASE or EGS_GLN_ID = ZEO_ZEC_ADD
	--[CHANGED] rkv0 '2021-08-20 15:32' изменил на 3 месяца (одного часто не хватает).
    --where ZEO_AUDIT_DATE > dateadd(MM,-1,cast(convert(char(8), CURRENT_TIMESTAMP, 112) as smalldatetime))
