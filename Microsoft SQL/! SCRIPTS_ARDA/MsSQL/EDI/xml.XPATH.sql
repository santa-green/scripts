...
--tArtist
SELECT * FROM tArtist
SELECT xmlData.query('/albums/album[2]/labels/label[1]') FROM tArtist --WHERE [name] = 'Radiohead'
SELECT xmlData.query('/albums/album/description[contains(lower-case(.),"record")]') FROM tArtist --WHERE [name] = 'Radiohead'
SELECT xmlData.value('(./albums/album/song/@title)[1]', 'nvarchar(100)') FROM tArtist --WHERE [name] = 'Radiohead'
SELECT xmlData.value('concat("00:", /albums[1]/album[1]/song[1]/@length)', 'time(0)') FROM tArtist --WHERE [name] = 'Radiohead'
SELECT [name], m.xmlData.exist('/albums/album/song[@title = "Garden of Eden"]') FROM tartist m
SELECT [name], m.xmlData.exist('/albums/album/song/@length[
                                                            (
                                                            if (fn:string-length(.) = 4)
...
--SELECT [name], m.xmlDate.modify() FROM tArtist m
SELECT m.xmlData.value('(./albums/album[@title="OK Computer"]/labels/label/text())[1]', 'nvarchar(100)') FROM tArtist m
SELECT m.xmlData.value('(./albums/album[2]/labels/label)[1]', 'nvarchar(100)') FROM tArtist m
WHERE m.[name] = 'Radiohead'

UPDATE tArtist SET xmlData.modify('replace value of (/albums/album[@title="OK Computer"]/labels/label/text())[1] with "Capitol" ') WHERE [name] = 'Radiohead'
UPDATE tArtist SET xmlData.modify('replace value of (/albums/album[@title="OK Computer"]/labels/label/text())[2] with "Parlophone" ') WHERE [name] = 'Radiohead'
SELECT m.xmlData.query('/albums/album[@title="OK Computer"]/labels') FROM tArtist m
SELECT xmlData.query('/albums/album[@title="OK Computer"]/labels')
--поменять местами элементы "label" без изменения их значений. Просто вставим копию первого лейбла в конец списка лейблов.
UPDATE tArtist SET xmlData.modify('insert <label>Parlophone</label> as last into 
    (/albums/album[@title="OK Computer"]/labels)[1]')

UPDATE dbo.tArtist
SET xmlData.modify('delete (/albums/album[@title="OK Computer"]/labels/label1)[1]')
WHERE name = 'Radiohead';

UPDATE tArtist SET xmlData.modify('insert attribute isPopular {"1"}
    into (/albums/album[1]/song[@title="Perfect Crime"])[1]')
WHERE [name] = 'Guns N'' Roses'

UPDATE tArtist SET xmlData.modify('insert (attribute Recorded {"1991"}, attribute Released {"1994-01-17"})
    into (/albums/album[2]/song[@title="Estranged"])[1]')
WHERE [name] = 'Guns N'' Roses'
SELECT * FROM tArtist
...
--at_z_FilesExchange
SELECT FileData.query('DECLAR/DECLARHEAD[1]'), * FROM at_z_FilesExchange WHERE ChID = 85923
SELECT FileData.query('DECLAR/DECLARHEAD/SOFTWARE[contains(., "COMDOC")]'), * FROM at_z_FilesExchange WHERE ChID = 85923
SELECT FileData.value('(/DECLAR/DECLARBODY/RXXXXG3S/@ROWNUM)[2]','nvarchar(100)'), * FROM at_z_FilesExchange WHERE ChID = 85923
...
--первые две песни каждого альбома для каждой из групп.
select name as artist,
    album.value('@title', 'varchar(50)') as album,
    song.value('@title', 'varchar(50)') as song
from tArtist A
