USE [Elit]
GO
/****** Object:  StoredProcedure [dbo].[ap_EXITE_CreateMessage]    Script Date: 10.12.2019 11:19:47 ******/
SET ANSI_NULLS ON
GO
...
/*
DECLARE @OutChID INT, @ErrMsg VARCHAR(250); exec [ap_EXITE_CreateMessage] @MsgType ='PACKAGE_METRO', @DocCode = 11012, @ChID = 200322563, @OutChID = @OutChID OUTPUT, @ErrMsg = @ErrMsg OUTPUT; SELECT @OutChID, @ErrMsg
SELECT * FROM at_z_FilesExchange WHERE StateCode IN (502,503) ORDER BY 3 DESC
SELECT * FROM at_z_FilesExchange WHERE StateCode IN (402) AND FILENAME LIKE '%PACKAGE%' ORDER BY 3 DESC
