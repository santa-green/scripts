...
set
--rvk0 '2019-08-21 11:51' вместо статично прописанных кодов используем функцию [af_GetQtyPack_By_ExtProdID]
--SELECT * FROM OPENQUERY ([s-sql-d4],'SELECT [elit].[dbo].af_GetQtyPack_By_ExtProdID(''1135889'', 7138)');

ZEP_POS_QTY = ZEP_POS_QTY * (SELECT * FROM OPENQUERY ([s-sql-d4],'SELECT [elit].[dbo].af_GetQtyPack_By_ExtProdID(''1135889'', 7138)'))

DECLARE @ZEP_POS_KOD nvarchar(100) = '1135889';
DECLARE @SQL nvarchar(4000);
DECLARE @result nvarchar(4000) = 'test';
...
Set @ZEP = @ZEP * (SELECT * FROM OPENQUERY ([s-sql-d4],'SELECT [elit].[dbo].af_GetQtyPack_By_ExtProdID(''1135889'', 7138)'));
select @ZEP
