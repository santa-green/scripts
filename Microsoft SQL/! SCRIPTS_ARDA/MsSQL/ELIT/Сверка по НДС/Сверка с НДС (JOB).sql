...
WHERE TaxPayer = 0
    ...
    --[ADDED] '2020-03-02 17:21' rkv0 убираем те даты регистрации плательщика, которые еще не наступили.
--    AND CAST(t.dat_reestr AS datetime) <= GETDATE()
    AND ISDATE(dat_reestr) = 1 AND t.dat_reestr != '2020-03-11'    
...
WHERE TaxPayer = 0
    ...
    AND EXISTS 
        (SELECT * FROM Elit.dbo.at_z_Contracts atz WHERE atz.CompID = rc.CompID AND atz.OurID IN (1,3) AND atz.ContrTypeID = 1 AND atz.EDate >= GETDATE())
    AND dat_anul IS NULL
--    SELECT * FROM r_Comps WHERE CompID = 71532
UNION ALL
