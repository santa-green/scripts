...
DECLARE @DocDate SMALLDATETIME = '20200901' --дата Прихода (один раз в месяц с 16 по 30)
...
SELECT * FROM t_Rec where DocDate = '20200201'
--DELETE t_Rec where DocDate = '20200101'
...
--проверка на пусты значения в поле Qty
IF exists(SELECT * FROM #tmp WHERE Qty IS NULL )
	BEGIN
	  SELECT * FROM #tmp WHERE Qty IS NULL 
	  RAISERROR ('ВНИМАНИЕ!!! В приходе %d в поле есть Qty нулевые значения!', 18, 1,@Num_Rec)
	  ROLLBACK
	  RETURN
	END   

--проверка на пусты значения в поле PPID
IF exists(SELECT * FROM #tmp WHERE PPID IS NULL )
	BEGIN
	  SELECT * FROM #tmp WHERE PPID IS NULL 
	  RAISERROR ('ВНИМАНИЕ!!! В приходе %d в поле есть PPID нулевые значения!', 18, 1,@Num_Rec)
	  ROLLBACK
	  RETURN
	END   
