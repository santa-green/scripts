USE [ElitR]
GO
/****** Object:  StoredProcedure [dbo].[ap_rkeeper_menu_check]    Script Date: 09.02.2021 17:03:17 ******/
SET ANSI_NULLS ON
GO
...
/*	--загружаем старое меню Ркипера из шаблона menu_R_keeper_old.xlsx
	...
	--загружаем полное меню Ркипера из шаблона menu_R_keeper.xlsx
...
BEGIN
	...
	, [для акций], [для предчеков], Классификация, [Отдел ФР], [Сервис печать], Категория, Группа
	 ...
	 FROM #Baze p WHERE p.ProdID NOT IN (SELECT [Внешний код] FROM #Menu) and ProdName like '%ДОБАВЬТЕ "r01 - Короткое название для RKeeper" во вкладку Значения%'
END
