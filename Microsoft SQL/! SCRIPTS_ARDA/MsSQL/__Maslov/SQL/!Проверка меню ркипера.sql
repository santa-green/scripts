...
BEGIN
	SELECT 'Товары, которые должны быть в RKeeper' 'проверка'
...
BEGIN
	...
	SELECT * FROM #Baze WHERE  cast(REPLACE([Название],'#','') as varchar(40)) <> cast(ProdName as varchar(40))
END

IF EXISTS(
	SELECT TOP 1 1 FROM #Menu m WHERE NOT EXISTS(SELECT VarValue FROM r_ProdValues WHERE ProdID = [Внешний код])
) 
BEGIN
     SELECT 'Не сходится названия товаров' 'Проверка', * FROM #Menu m WHERE NOT EXISTS(SELECT VarValue FROM r_ProdValues WHERE ProdID = [Внешний код])
END
