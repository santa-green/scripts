ALTER FUNCTION [dbo].[af_JSON_convert_to_JSON_string]
	(
	...
	/*
	ВАЖНО!!!!
	Перед созданием этой функции нужно создать тип данных Hierarchy попмощью кода ниже.
	------------------------------Выполнить код----------------------------------------
	  ...
	  WHILE @ii>0
	    ...
	    SELECT  
	        ...
	        WHEN 'object' then '  {'+@CrLf+SPACE(@indent+2)
	           +'@Object'+CONVERT(VARCHAR(5),TheRow.[OBJECT_ID])+@CrLf+SPACE(@indent+2)+'}'
	        WHEN 'string' THEN '"'+dbo.af_JSON_Escaped(TheRow.StringValue)+'"'
