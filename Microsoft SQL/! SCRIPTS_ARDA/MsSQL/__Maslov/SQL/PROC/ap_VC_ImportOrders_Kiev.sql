ALTER PROCEDURE [dbo].[ap_VC_ImportOrders_Kiev]
AS
BEGIN
  --для отладки 
  ...
  SET NOCOUNT ON
  SET XACT_ABORT ON
--pvm0 30.08.17 16.40 тепер признак 5 берется из универсального справочника с кодом 1000000006
--moa0 10.04.2019 13:37 настроил процедуру для работы с базой 3.16

--BEGIN TRAN TranOrders

--exec ap_VC_ImportOrders_Kiev
  ...
  DECLARE 
   ...
   @RegionID INT = 2,--регион (Днепр 1 прайс 28; Киев 2 прайс 48; Одесса 3 прайс 45; Харьков 5 прайс 40; )
   @DBNameOPT VARCHAR(250) = '[S-SQL-D4].Elit',
   @AddressIM VARCHAR(250) = 'магазин "Vintage"; м. Київ, вул. Павла Тичини, буд.1в'
      ...
      BEGIN--OrderW
		...
		BEGIN-- если ОПТ
		  ...
		  SELECT
		    ...
		    124 CodeID4,
		    --pvm0 30.08.17 16.40 тепер признак 5 берется из универсального справочника с кодом 1000000006(CASE RegionID WHEN 1 THEN 997 WHEN 2 THEN 988 WHEN 3 THEN 971 WHEN 5 THEN 987 END)
		...
		BEGIN--IF @RangeID = 1
		  ...
		  BEGIN--OrderW_1   
			...
			--для отладки
			PRINT N'@InvChID, @DocID, @DocID, @ExpDate, @KursMC, @OurID, @InvStockID, 10791, 63, 18, 4, 0, 0, 0, 1, 0, 0,@ExpDate ,@OrdID, 2, 202, 1, ''магазин "Vintage"; м. Київ, вул. Павла Тичини, буд.1в'''
			SELECT @InvChID, @DocID, @DocID, @ExpDate, @KursMC, @OurID, @InvStockID, 10791, 63, 18, 4, 0, 0, 0, 1, 0, 0,@ExpDate ,@OrdID, 2, 202, 1, @AddressIM
					 ...
					 BEGIN--IF @DCardID ELSE  
						...
						BEGIN--IF @OrdPrice != @PurPrice                                        
						 ...
						 IF (NOT EXISTS (SELECT * FROM dbo.r_ProdMP WITH(NOLOCK) 
							 ...
							 WHERE DCardID = @DCardID
							 AND Discount != ROUND((1 - (@OrdPrice / @PurPrice))  * 100, 0)))
						 BEGIN--IF 01
						   IF NOT EXISTS (SELECT * FROM dbo.z_DocDC WITH(NOLOCK) WHERE DocCode = 1011 AND ChID = @SaleChID AND DCardChID = @DCardChID)
							 INSERT dbo.z_DocDC 
							   (DocCode, ChID, DCardChID)
		  ...
		  INSERT dbo.at_t_IORes
		    ...
		    SELECT
			  ...
			  (CASE PayFormCode WHEN 1 THEN (CASE RegionID WHEN 1 THEN 24 WHEN 2 THEN 74 WHEN 5 THEN 76 END) 
						WHEN 2 THEN 19 ELSE -1 END) CodeID3,                 
			  124 CodeID4,
			  --pvm0 30.08.17 16.40 тепер признак 5 берется из универсального справочника с кодом 1000000006(CASE RegionID WHEN 1 THEN 997 WHEN 2 THEN 988 WHEN 3 THEN 971 WHEN 5 THEN 987 END) CodeID5, 
		      ISNULL((SELECT top 1 CAST(RefName AS INT) FROM r_Uni WHERE RefTypeID = 1000000006 AND ISNUMERIC(RefName) = 1 AND RefID = RegionID), 0) CodeID5, 
