ALTER PROCEDURE [dbo].[ip_ImpFromTSD_Oll_T_rec] @Type int, @MSGOUT varchar(8000) OUTPUT	
AS
BEGIN
/*Импорт всех документов с ТСД*/
--pvm0 13.07.2017 для автоматической обработки товаров которых не было в заказе
--(pvm0 2018-03-26 16:16) теперь импортируются только те документы которые на которые есть заказы 
--(pvm0 2019-02-07 13:06) теперь импортируются только те документы в которых кол заказаных и кол принятых товаров больше нуля
--(pvm0 2019-02-25 11:30) теперь импортируются только те документы в которых указан код предприятия 
--Maslov '2019-04-11 14:32:13.751' дата партии берется из даты документа в ТСД

/*
BEGIN TRAN
	DECLARE @MSGOUT VARCHAR(8000)
	EXEC ip_ImpFromTSD_Oll_T_rec 0, @MSGOUT OUTPUT
...
/*старый способ определения складов
...
IF SUSER_SNAME() = 'kea20' insert #sklad select StockID  from r_Stocks where StockID in (1200,1201,1203,1204,1205,1256,1257,1327)
IF SUSER_SNAME() = 'zoa3' insert #sklad select StockID  from r_Stocks where StockID in (1200,1201,1203,1204,1205,1256,1257,1327)
--IF HOST_NAME () = 'GA_KH20' insert #sklad select StockID  from r_Stocks where StockID in (1252)
IF HOST_NAME () = 'VINTAGE-KH1' insert #sklad select StockID  from r_Stocks where StockID in (1245)
IF HOST_NAME () = 'ELIT_KIEV23' insert #sklad select StockID  from r_Stocks where StockID in (1241)/*Киев новый маркетвин фирма 6*/
IF SUSER_SNAME() = 'paa18' insert #sklad select StockID  from r_Stocks where StockID in (1252,1245)--для Харькова пользователь paa18

--новый способ определения складов (pvm0 2018-02-08 10:22)
IF ISNULL(IS_MEMBER('Харьков Экономист'),0) = 1  INSERT #sklad SELECT StockID  FROM r_Stocks WHERE StockID IN (1252,1245)
IF ISNULL(IS_MEMBER('Киев Экономист'),0) = 1  INSERT #sklad SELECT StockID  FROM r_Stocks WHERE StockID IN (1241)
IF ISNULL(IS_MEMBER('Днепр Экономист'),0) = 1  INSERT #sklad SELECT StockID  FROM r_Stocks WHERE StockID IN (1200,1201,1203,1204,1205,1256,1257,1327)
...
IF 1=0 -- для теста
SELECT doc_type, id_doc FROM it_TSD_doc_head 
WHERE doc_type IN (@Type) and [dbo].[zf_MatchFilterInt](sklad_out_id,[dbo].[af_GetStocksUser](),',') = 1
AND id_doc NOT IN  (SELECT Id_doc FROM it_TSD_doc_details GROUP BY Id_doc HAVING SUM (Count_real)=0 )
--(pvm0 2019-02-07 13:06) теперь импортируются только те документы в которых кол заказаных и кол принятых товаров больше нуля 
AND id_doc NOT IN  (SELECT Id_doc FROM it_TSD_doc_details GROUP BY Id_doc HAVING SUM (Count_doc)=0 )
...
AND id_doc IN (SELECT ChID FROM t_EOExp)

SET @NumDocs = ''
SET @Logs = ''
DECLARE DocType_IDDoc CURSOR FAST_FORWARD 
FOR 
--(pvm0 2018-02-09 11:25) теперь склады определяяются через функцию [af_GetStocksUser]
--(pvm0 2018-03-26 16:16) теперь импортируются только те документы которые на которые есть заказы 
SELECT doc_type, id_doc FROM it_TSD_doc_head 
WHERE doc_type IN (@Type) and [dbo].[zf_MatchFilterInt](sklad_out_id,[dbo].[af_GetStocksUser](),',') = 1
AND id_doc NOT IN  (SELECT Id_doc FROM it_TSD_doc_details GROUP BY Id_doc HAVING SUM (Count_real)=0 )
...
OPEN DocType_IDDoc 
FETCH NEXT FROM DocType_IDDoc INTO @doc_type, @id_doc
...
BEGIN	
	...
	BEGIN
		...
		SELECT @NumDocs = @NumDocs  + case @NumDocs when '' then '' else ',' end  + '' + CAST(@NewDocID AS VARCHAR(20))
		SELECT @Logs = @Logs  + case @Logs when '' then '' else '@' end  + 
		+ 'N П:' + CAST(@NewDocID AS VARCHAR(20))+ '|' +
		+ 'N З:' + CAST(@id_doc AS VARCHAR(20))+ '|' +
		+ 'Ф:' + CAST(@OurID AS VARCHAR(20))+ '|' +
		+ 'С:' + CAST(@StockID AS VARCHAR(20))+ '|' +
		+ 'Пр:' + CAST(@CompID AS VARCHAR(20))
...
/*Вставка детальной части*/
		...
		BEGIN
			...
			IF EXISTS (SELECT TOP 1 1 FROM r_ProdMQ WHERE BarCode = @BarCode)
				BEGIN		
					...
					--SELECT @Price = PriceCC --Цена ВС
					SELECT @Price = PriceAC --Цена ЛВ
					FROM t_EOExpd WHERE ProdID = @Id_good AND ChID = @id_doc
	...
	IF 1=0--для теста
	SELECT @Id_good ProdID, @RecPPID PPID, (@Price / @KursMC) PriceMC_In, 0 PriceMC, @RecPPID Priority
	--, @ExpDate ProdDate
	--Maslov '2019-04-11 14:32:13.751' дата партии берется из даты документа в ТСД
	,(SELECT TOP 1 doc_date FROM it_TSD_doc_head WHERE id_doc =@id_doc ) 
	, ''DLSDate, @CurrID CurrID, 
				 ...
				 @Price PriceCC_In, @Price CostCC, 0 PPDelay, '' ProdPPDate,  @Price PriceAC_In, (@Price / @KursMC) CostMC, ''CstProdCode, NULL ElitProdID
                  ...
                  (ProdID, PPID, PriceMC_In, PriceMC, Priority, ProdDate, DLSDate, CurrID, CompID, Article, CostAC, PPWeight, File1, File2, File3,
                   ...
                   SELECT
                   @Id_good ProdID, @RecPPID PPID, (@Price / @KursMC) PriceMC_In, 0 PriceMC, @RecPPID Priority
				   --, @ExpDate ProdDate
				   --Maslov '2019-04-11 14:32:13.751' дата партии берется из даты документа в ТСД
				   ,(SELECT TOP 1 doc_date FROM it_TSD_doc_head WHERE id_doc =@id_doc ) ProdDate
				   , ''DLSDate, @CurrID CurrID, 
					@CompID CompID, '' Article, @Price CostAC, 0 PPWeight,NULL File1, NULL File2, NULL File3, 
				 @Price PriceCC_In, @Price CostCC, 0 PPDelay, '' ProdPPDate,  @Price PriceAC_In, (@Price / @KursMC) CostMC, ''CstProdCode, NULL ElitProdID
				...
				*/   
				INSERT dbo.t_RecD
                  (ChID, SrcPosID, ProdID, PPID, UM, Qty, 
                  PriceCC_nt, SumCC_nt, 
                  Tax, TaxSum, 
                  PriceCC_wt, SumCC_wt, CostSum, CostCC, Extra, PriceCC, BarCode, SecID)
                  VALUES
                  (@NewChID, @SrcPosID, @Id_good, @RecPPID, @UM, @Count_real, 
                   dbo.zf_GetProdPrice_nt(@Price, @Id_good, @ExpDate), dbo.zf_GetProdPrice_nt(@Price, @Id_good, @ExpDate) * @Count_real,
                   dbo.zf_GetProdPrice_wtTax(@Price, @Id_good, @ExpDate), dbo.zf_GetProdPrice_wtTax(@Price, @Id_good, @ExpDate) * @Count_real, 
                   @Price, @Price * @Count_real, 0, 0, 0, 0, @BarCode, 1)

                  /*делал округление 21,02,2018 не прижилось, вернул обратно 22,02,2018
                  (@NewChID, @SrcPosID, @Id_good, @RecPPID, @UM, @Count_real, 
                   Round(dbo.zf_GetProdPrice_nt(@Price, @Id_good, @ExpDate),2), Round(dbo.zf_GetProdPrice_nt(@Price, @Id_good, @ExpDate),2) * @Count_real,
                   Round(dbo.zf_GetProdPrice_wtTax(@Price, @Id_good, @ExpDate),2), Round(dbo.zf_GetProdPrice_wtTax(@Price, @Id_good, @ExpDate),2) * @Count_real, 
                   @Price, @Price * @Count_real, 0, 0, 0, 0, @BarCode, 1)*/
...
SET @MSGOUT = 'Созданы приходы №: ' + ISNULL(@NumDocs,'ни одного') + ' @ Общее количество приходов: ' + CAST(@CountDocs AS VARCHAR(20)) + '@' + @Logs

INSERT at_z_LogTSD (OperDesc, LogMsg, UserName) VALUES ('Кнопка Импорт приходов (ip_ImpFromTSD_Oll_T_rec)',@MSGOUT,SUSER_SNAME())
...
SELECT * FROM at_z_LogTSD ORDER BY 1
SELECT * FROM t_rec where docid in (600000308, 600000314,600019169,600019170)
