...
BEGIN
	...
	SELECT * 
	 INTO #Menu_old	
	FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', N'Excel 12.0; IMEX=1; Database=\\s-sql-d4\OT38ElitServer\Import\R_keeper\menu_R_keeper — копия.xlsx' , 'select * from [Лист1$]') as ex
	WHERE  Статус = 'Активный'
...
/*
SELECT * FROM #Baze WHERE ProdID in (607897)
SELECT * FROM #Baze WHERE ProdName like '%шей%'
*/

--товары которые должы быть в ркипере
...
--товары которых не должно быть в ркипере
...
----/*проверка цен разных по 76 прайсу болоньез*/
