...
DECLARE	-- укажите параметры
		 @StockID VARCHAR(MAX) = '11,311' -- склады на которые делается заказ
...
--SELECT * FROM t_rem m WITH(NOLOCK) where ProdID in (33106) and StockID in ( select AValue from zf_FilterToTable(@StockID) )
