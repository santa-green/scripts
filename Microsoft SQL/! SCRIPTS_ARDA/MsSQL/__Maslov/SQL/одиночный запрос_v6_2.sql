--запрос по логистике версия 6_2 от 18.09.2018 10:15
SET NOCOUNT ON
DECLARE	-- укажите параметры
		 @StockID VARCHAR(MAX) = ',330' -- склады на которые делается заказ
...
IF 1=1 --#Month2
BEGIN
SET @BeginDate = DATEADD(DAY , -@Months*@Interval, dbo.zf_GetDate( GETDATE() ))
SET @EndDate = DATEADD(DAY, (-@Months+1)*@Interval, dbo.zf_GetDate( GETDATE() ))
