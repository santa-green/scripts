...
GO
select ReplicaEventID, ReplicaSubCode, ExecStr as 'ƒнепр, локальный сервер [S-MARKETA].ElitV_DP 192.168.70.2', Status, Msg, DocTime 
...
update z_replicain 
set status = 1
where status != 1 and ReplicaEventID in (15578277)

==================================================================

--1. лечение. пропусть эту реплику на сетевом сервере 
update [s-marketa].elitv_dp.dbo.z_replicain set status = 1 where status != 1 and ReplicaEventID in (15578277)
update [s-marketa3].ElitRTS301.dbo.z_replicain set status = 1 where status != 1 and ReplicaEventID in (15578275)
update [s-marketa4].ElitRTS181.dbo.z_replicain set status = 1 where status != 1 and ReplicaEventID in (15578275)
update [192.168.174.30].ElitRTS201.dbo.z_replicain set status = 1 where status != 1 and ReplicaEventID in (15578276)
update [192.168.157.22].ElitRTS302.dbo.z_replicain set status = 1 where status != 1 and ReplicaEventID in (15578275)
update [192.168.174.38].ElitRTS220.dbo.z_replicain set status = 1 where status != 1 and ReplicaEventID in (15578275)
==================================================================
