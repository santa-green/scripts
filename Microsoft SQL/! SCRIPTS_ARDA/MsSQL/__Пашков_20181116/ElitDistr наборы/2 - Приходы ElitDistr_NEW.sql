...
DECLARE @DocDate SMALLDATETIME = '2018-11-01' --дата Прихода (один раз в месяц)
DECLARE @ROLLBACK_TRAN  int = 1       -- 1 тестирование, выполнится ROLLBACK TRAN   0 рабочий режим, выполнится COMMIT TRAN

/*
SELECT * FROM t_Rec where ChID = 41
SELECT * FROM t_Recd where ChID = 41

SELECT * FROM t_Rec where DocDate = '2018-11-01'
--DELETE t_Rec where DocDate = '2018-11-01'

--текущие остатки
SELECT OurID, StockID, SecID, ProdID, PPID, Qty, AccQty FROM t_rem where OurID = 1 and StockID = 20 and (Qty <> 0 or AccQty <> 0) ORDER BY 1,2,3,4,5,6,7
*/
