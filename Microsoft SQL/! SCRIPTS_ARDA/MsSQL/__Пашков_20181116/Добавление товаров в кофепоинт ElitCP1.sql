--Добавление товаров в кофепоинт ElitCP1

--USE ElitCP1 -- --CofePoint DP 192.168.42.6
...
insert r_ProdC
	SELECT ChID, PCatID, PCatName, Notes, 0 CodeID1, 0 Priority FROM [S-SQL-D4].ElitR.dbo.r_ProdC where PCatID not in (SELECT PCatID FROM r_ProdC)
SELECT * FROM r_ProdC

insert r_ProdG
	SELECT * FROM [S-SQL-D4].ElitR.dbo.r_ProdG where PGrID not in (SELECT PGrID FROM r_ProdG) and PGrID in( 50061,50061,50061,50061,50061,50119,50119,50086,50034,50034,50033,50033)
SELECT * FROM r_ProdG

insert r_ProdG1
	SELECT ChID, PGrID1, PGrName1, Notes FROM [S-SQL-D4].ElitR.dbo.r_ProdG1 where PGrID1 not in (SELECT PGrID1 FROM r_ProdG1) --and PGrID1 in( 208)
SELECT * FROM r_ProdG1

insert r_ProdG2
	SELECT * FROM [S-SQL-D4].ElitR.dbo.r_ProdG2 where PGrID2 not in (SELECT PGrID2 FROM r_ProdG2)-- and PGrID2 in( 40012)
SELECT * FROM r_ProdG2

insert r_ProdG3
	SELECT * FROM [S-SQL-D4].ElitR.dbo.r_ProdG3 where PGrID3 not in (SELECT PGrID3 FROM r_ProdG3) and PGrID3 in( 22)
SELECT * FROM r_ProdG3

insert r_prods
	SELECT ChID, ProdID, ProdName, UM, Country, Notes, PCatID, PGrID, Article1, Article2,
	 Article3, Weight, Age, PriceWithTax, Note1, Note2, Note3, MinPriceMC, MaxPriceMC, 
	 MinRem, CstDty, CstPrc, CstExc, StdExtraR, StdExtraE, MaxExtra, MinExtra, UseAlts, 
	 UseCrts, PGrID1, PGrID2, PGrID3, PGrAID, PBGrID, LExpSet, EExpSet, InRems, IsDecQty,
	  File1, File2, File3, AutoSet, Extra1, Extra2, Extra3, Extra4, Extra5, Norma1, Norma2, 
	  Norma3, Norma4, Norma5, RecMinPriceCC, RecMaxPriceCC, RecStdPriceCC, RecRemQty, 
	  InStopList, PrepareTime, AmortID, PGrID4, UseDiscount, ScaleGrID, ScaleStandard,
	   ScaleConditions, ScaleComponents, TaxFreeReason, CstProdCode, WeightGr, WeightGrWP,
		BoxVolume, TaxTypeID FROM [S-SQL-D4].ElitR.dbo.r_prods 
...
insert r_ProdMP
	SELECT ProdID, PLID, PriceMC, Notes, CurrID, BDate, EDate, InUse, isnull(PromoPriceCC,0)  FROM [S-SQL-D4].ElitR.dbo.r_ProdMP 
...
insert t_PInP
	SELECT ProdID, PPID, PPDesc, PriceMC_In, PriceMC, Priority, ProdDate, CurrID, 
	CompID, Article, CostAC, PPWeight, File1, File2, File3, PriceCC_In, CostCC, PPDelay, 
	ProdPPDate, DLSDate, IsCommission, CostMC, PriceAC_In, 0 FEAProdID, 0 ProdBarCode, 
	CstDocCode, 0 CstDocDate, CstProdCode, ParentDocCode, ParentChID 
