--запрос по логистике версия 6_1 от 16.08.2018 10:00
DECLARE	-- укажите параметры
		 @StockID VARCHAR(MAX) = '4,304' -- склады на которые делается заказ
		,@StockID_ALL VARCHAR(MAX) = '4,304,11,311,27,327,85,385,23,323' -- все склады на которые делаются заказы
		...
		,@Months INT = 2
/*история изменений скрипта
версия 4 - появился параметр @AllProds который позваляет выбирать любые товары
версия 5 - появился столбец Расход220320 - это расход по складу 220 и 320
версия 6 -

ТЗ 6 
1-расходы разбить помесячно было расход -> расход_m1 и расход_m2 
1-расходы разбить помесячно было Расход220320 -> Расход220320_m1 и Расход220320_m2 
1-расходы разбить помесячно было Расход220320_seti -> Расход220320_seti_m1 и Расход220320_seti_m2
...
;WITH 
	...
	,gr_t_Ret220320_CTE AS(
					SELECT d.ProdID, SUM(d.Qty) SumQty FROM t_Ret m WITH(NOLOCK)
					JOIN t_RetD d WITH(NOLOCK) ON d.ChID = m.ChID
					WHERE m.DocDate  BETWEEN @BeginDate AND @EndDate and m.StockID in ( 220,320 )	and m.CodeID1 <> 100 GROUP BY d.ProdID
					)
...
--расход общий
	,gr_t_Ret_CTE_global AS(
					SELECT d.ProdID, SUM(d.Qty) SumQty FROM t_Ret m WITH(NOLOCK)
					JOIN t_RetD d WITH(NOLOCK) ON d.ChID = m.ChID
					WHERE m.DocDate  BETWEEN @BeginDate AND @EndDate and m.StockID in ( select AValue from zf_FilterToTable(@StockID_ALL) )	and m.CodeID1 <> 100 GROUP BY d.ProdID
					)
...
SELECT PCatName 'Категория', PGrName1 'Проект', PGrAID 'Код группы альтернатив', ProdID 'Код товара', ProdName 'Наименование товара', Кол_в_ящике, Остаток220_бр 'Остаток220', Резерв220 'Резерв_ЦС', Остаток218_бр 'Остаток218', Остаток, Расход_m1 'Расход_посл_месяц', Расход_m2 'Расход_пред месяц', Расход220320_seti_m1 'Расход_СЕТИ_посл месяц', Расход220320_seti_m2 'Расход_СЕТИ_пред месяц', ROUND (Прогноз,0) 'Прогноз', ROUND (Доли_склада,1) 'Доли_склада', кратный_Заказ_по_прогнозу 'Заказ по прогнозу по ящ', Кор_Кратный_Факт_заказ 'Факт_заказ'
FROM (
	SELECT *
	,CASE WHEN Остаток220_бр < Кратный_Факт_заказ THEN Остаток220_бр ELSE Кратный_Факт_заказ END as 'Кор_Кратный_Факт_заказ' 
	...
	FROM (
		...
		FROM (
			...
			m1.Остаток_на_ЦС_бр, m1.Резерв_на_ЦС, m1.Остаток220_бр,m1.Резерв220, m1.Остаток218_бр, m1.Резерв218, m1.Остаток, 
