...
/* Приход товара по ГТД: Заголовок */
    SELECT DISTINCT t_Cst.OurID OurID, t_Cst.StockID StockID, t_CstD.SecID SecID, t_CstD.ProdID ProdID, t_CstD.PPID PPID, SUM(t_CstD.Qty) Qty, 0 AccQty    FROM r_Prods.r_Prods WITH (NOLOCK), t_CstD WITH (NOLOCK), t_Cst WITH(NOLOCK)     Where ProdID  = @ProdID and  t_CstD.ProdID = r_Prods.ProdID AND t_Cst.ChID = t_CstD.ChID AND (r_Prods.InRems <> 0) AND (t_Cst.DocDate BETWEEN @BDate AND @EDate)
