...
--/*для теста*/
----IF HOST_NAME () = 'S-ELIT-DP' 
insert #sklad select StockID  from r_Stocks where StockID in (1200,1201,1203,1204,1205,1256,1257,1327)
...
FROM t_EOExp WHERE ChID = @id_doc
...
select @Id_good Id_good, @Count_real Count_real
			...
			IF EXISTS (SELECT TOP 1 1 FROM r_ProdMQ WHERE BarCode = @BarCode)
				BEGIN		
					...
					SELECT @SrcPosID = (SELECT ISNULL(MAX(SrcPosID)+1,1) 
						                FROM t_RecD 
								        WHERE ChID = @NewChID)
SELECT @SrcPosID SrcPosID								
					IF @Count_real > 0 
					BEGIN
				
					SELECT @RecPPID = dbo.tf_NewPPID(@Id_good)
SELECT @RecPPID	RecPPID		
...
SELECT * FROM t_EOExp WHERE ChID = @id_doc

SELECT * FROM t_EOExpD WHERE ChID = @id_doc
