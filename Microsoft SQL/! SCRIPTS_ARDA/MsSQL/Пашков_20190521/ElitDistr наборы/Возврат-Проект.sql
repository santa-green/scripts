--USE [Elit]
--GO
--/****** Object:  StoredProcedure [dbo].[ap_CalcPRet]    Script Date: 04.01.2019 9:09:21 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO
--ALTER PROCEDURE [dbo].[ap_CalcPRet] @ChID INT
--AS
--BEGIN
  --SET NOCOUNT ON;
  --SET XACT_ABORT ON;
  ...
  DECLARE @ChID INT = 1
  DECLARE @ProdID INT = 26137
  DECLARE @OurID INT, @CompID INT, @CompAddID INT, @BDate SMALLDATETIME, @EDate SMALLDATETIME, @CForm INT;
  DECLARE @PriceCC NUMERIC(21,9), @ProdPPDate SMALLDATETIME, @PriceWithTax BIT;
  DECLARE @AChID INT;
  ...
  SELECT 
   ...
   @CompAddID = 0,
   @BDate = '2017-02-01',
   @EDate = dbo.zf_GetDate(GETDATE()),
   @CForm = 1, -- 1 - безнал
   @PriceWithTax = 1 -- 1 - цена включает НДС
  
   --SELECT * FROM at_t_PRet WITH(NOLOCK)  WHERE ChID = 3950;
   --SELECT * FROM at_t_PRetA WITH(NOLOCK)  WHERE ChID = 3950;
   --SELECT @ProdID ProdID, 1000 Qty, 0 PriceCC, null ProdPPDate FROM at_t_PRetA WITH(NOLOCK)  WHERE ChID = 3950;
...
--BEGIN TRAN
  ...
  FROM (SELECT 1 ChID, @ProdID ProdID, 1000 Qty, 0 PriceCC, null ProdPPDate, 2 AChID) f
	CROSS JOIN TInv m
  ...
  --SELECT * FROM T where ChID in (101535604)
  ...
  SELECT 
    ...
    m.TaxDocID, m.TaxDocDate, m.TaxPayer, m.CompID, dbo.aft_GetValidPPID(@ProdID, m.PPID) PPID, m.ProdPPDate,m.PriceCC, SUM(m.Qty) MaxQty
  FROM T m
...
--SELECT * FROM t_InvD where ChID in (101535604,200042281,200051247,200051247,200058680,200082728,200082728,200087748,200088518,200088518,200185959,200209089,200209166,200210298,200212337,200212498,200212523,200212523,200212523,200215324,200215428,200217255)
--and ProdID = 26137
--ORDER BY 1
