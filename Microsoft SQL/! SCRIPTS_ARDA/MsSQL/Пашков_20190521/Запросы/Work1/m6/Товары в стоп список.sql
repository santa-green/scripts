...
where ts.docdate >= CONVERT(DATETIME, '2008-01-01 00:00:00', 102) 
group by t.prodid

select td.prodid , sum (td.qty)as RecQty
into temp_t_recD
from   t_rec t inner join t_recD td  on t.chid= td.chid
where t.docdate >= CONVERT(DATETIME, '2008-01-01 00:00:00', 102) and t.compid = 268
group by td.prodid
...
from temp_t_rem  full join temp_t_Saled  on temp_t_rem.prodid = temp_t_Saled.prodid   
 ...
 where sumqty =0 and temp_t_Saled.prodid  is null and temp_t_recD .prodid is null and tn.compid =268
