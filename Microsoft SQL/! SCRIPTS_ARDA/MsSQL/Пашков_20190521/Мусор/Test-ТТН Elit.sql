...
@OurID int = 1
,@DocIDs int = 1182868
,@PrintCodeID4 int = 102
,@AChID int = 156--156	Кредит-Днепр  -  Нац сети - 2600530539322	0
,@PrintDriver int = 1

/*
[dq_Doc."FwName"][dq_Doc."FwAddress"]
[dq_Doc."CnName"][dq_Doc."CnAddress"]
*/

;WITH TCh (ChID) AS (SELECT ChID FROM dbo.t_Inv WHERE OurID = @OurID AND DocID IN (SELECT AValue FROM dbo.zf_FilterToTable(@DocIDs))),     
TDoc AS (                    
...
rc.Contract1 CnName, ', ' + LTRIM(RTRIM(rc.Address)) + ISNULL(', ' + NULLIF(LTRIM(RTRIM(rc.City)),''),'') + ISNULL(', ' + NULLIF(LTRIM(RTRIM(rc.Region)),''),'') + ISNULL(', ' + NULLIF(LTRIM(RTRIM(rc.PostIndex)),''),'') CnAddress, ', ЄДРПОУ ' + CASE WHEN LEN(ISNULL(rc.Code, '')) IN (8,10) THEN rc.Code END CnCode, ', ІПН: ' + CASE WHEN LEN(ISNULL(rc.TaxCode, '')) IN (10,12) THEN rc.TaxCode END CnTaxCode, ', р/р ' + dbo.af_GetFiltered(rcc.CompAccountCC) + ' в ' + rcb.BankName + ', МФО ' + CAST(rcb.BankID AS VARCHAR) CnBankReqs,
...
LEFT JOIN dbo.r_Codes4 rc4 WITH(NOLOCK) ON rc4.CodeID4 = m.CodeID4 AND rc4.CodeID4 != 0 AND 1 = @PrintCodeID4
...
JOIN dbo.r_OursCC occ WITH(NOLOCK) ON occ.OurID=ro.OurID AND occ.ChID = @AChID
...
LEFT JOIN dbo.at_r_Drivers rd WITH(NOLOCK) ON rd.DriverID = m.DriverID AND rd.DriverId != 0 AND 1 = @PrintDriver
...
--156	Кредит-Днепр  -  Нац сети - 2600530539322	0
