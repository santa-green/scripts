...
CREATE PROC [dbo].[ap_Export_J1201210] @ChID INT, @DocCode INT = 0, @testing int = null
AS 
...
--  DECLARE @ChID INT = (SELECT MAX(ChID) FRom t_Inv WHERE TaxDocID > 0  AND TSumCC_wt > 10000) 
  ...
  (
    [TAB1_N] VARCHAR(MAX), --номер позиции по порядку
    [SID] SMALLINT,
	[TAB1_PKID] VARCHAR(MAX),--код причины коректировки (102,103)
	[TAB1_N_GK] VARCHAR(MAX), --номер группы коректировки
...
-------
 ...
 DECLARE Cur Cursor for
   select    rtm.ChID, CompID,CodeID1, CodeID2 ,CodeID3, ProdID, PPID, Qty, PriceCC_nt, SrcTaxDocID,SrcTaxDocDate,tax  
   from  dbo.t_Ret rtm JOIN t_RetD rtD on rtm.ChID=rtD.ChID  where rtm.ChID=@Chid  
   group by rtm.ChID, CompID, CodeID1, CodeID2 ,CodeID3, ProdID, PPID, Qty, PriceCC_nt, SrcTaxDocID,SrcTaxDocDate, Tax
...
BEGIN
 ...
 --    [TAB1_N] INT, --номер позиции по порядку
 --   [SID] SMALLINT,
	--[TAB1_PKID] VARCHAR(MAX),--код причины коректировки (102,103)
	--[TAB1_N_GK] VARCHAR(MAX), --номер группы коректировки
...
SELECT 
0 [TAB1_N],
CASE WHEN (SUM(ISNULL(@QtyInv, 0)) - SUM(@Qty)) <>0 THEN  @iter   ELSE  @iter2 END   [SID],
CASE WHEN rtm.CodeID2 IN (43) THEN 101 WHEN rtm.CodeID2 IN (44) THEN 102 WHEN rtm.CodeID2 IN (19) THEN 103 WHEN rtm.CodeID2 IN (38) THEN 203 ELSE 0 END [TAB1_PKID],
0 [TAB1_N_GK],
 ...
 dbo.zf_DateToStr(@SrcTaxDocDate) [TAB1_A1],
  ...
  SELECT 
  0 [TAB1_N],
  CASE WHEN (SUM(ISNULL(@QtyInv, 0)) - SUM(@Qty)) <>0 THEN  @iter +1  ELSE  @iter2 END [SID],
  CASE WHEN rtm.CodeID2 IN (43) THEN 101 WHEN rtm.CodeID2 IN (44) THEN 102 WHEN rtm.CodeID2 IN (19) THEN 103 WHEN rtm.CodeID2 IN (38) THEN 203 ELSE 0 END [TAB1_PKID],
  0 [TAB1_N_GK],
...
DEALLOCATE Cur38
 ...
 --   [SID] SMALLINT,
    ...
    GROUP BY rtm.TaxDocDate, LTRIM(RTRIM(ISNULL(NULLIF(ec.ExtProdName,''), rp.Notes))), pp.CstProdCode, 
      rumq.RefName, rumq.RefID, rtd.PriceCC_nt, pp.CstDocCode, pp.CstDocDate, (CASE WHEN  rtm.CodeID1 = 63  and rtm.CodeID2 IN (18,43, 44, 69,96) and rtm.CodeID3 = 4   
		and rtm.taxdocdate >= '20161201'and rp.Article2 like '%&M%' and rtm.CompID Not between 7000 and 7999 and  rtm.CompID Not between 10790 and 10799 and r.CodeID4 != 5002  THEN  rp.Article2
...
begin
	...
	--SELECT ROW_NUMBER()OVER(ORDER BY [SID]) n ,RANK ()OVER(PARTITION BY TAB1_A3 ORDER BY TAB1_A3) gr,* FROM @DetTbl
	--SELECT ROW_NUMBER()OVER(ORDER BY [SID]) n ,RANK ()OVER(ORDER BY TAB1_A3) gr,* FROM @DetTbl ORDER BY 1
	SELECT ROW_NUMBER()OVER(ORDER BY [SID]) n ,DENSE_RANK ()OVER(ORDER BY TAB1_A3) gr,* FROM @DetTbl ORDER BY 1

update t set TAB1_N = n , TAB1_N_GK = gr from (SELECT ROW_NUMBER()OVER(ORDER BY [SID]) n ,DENSE_RANK ()OVER(ORDER BY TAB1_A3) gr,* FROM @DetTbl) t 	 
	SELECT * FROM @DetTbl
...
SELECT 1 '@TAB', [SID] '@LINE', PName '@NAME', PVal 'VALUE'
        ...
        UNPIVOT
          (PVal FOR PName IN
            (TAB1_A011,TAB1_A01,TAB1_A1,TAB1_A2,TAB1_A3,TAB1_A31,TAB1_A32,TAB1_A33,TAB1_A4,TAB1_A41,TAB1_A5,TAB1_A6,TAB1_A7,TAB1_A8,TAB1_A013,TAB1_N,TAB1_PKID,TAB1_N_GK)
       ) AS unpvt
  ...
  --обновление полей с номером позиции и номером групы
  update t set TAB1_N = n , TAB1_N_GK = gr from (SELECT ROW_NUMBER()OVER(ORDER BY [SID]) n ,DENSE_RANK ()OVER(ORDER BY TAB1_A3) gr,* FROM @DetTbl) t 	 
