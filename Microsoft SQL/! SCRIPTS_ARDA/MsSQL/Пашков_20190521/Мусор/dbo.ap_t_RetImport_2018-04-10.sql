USE [ElitDistr]
GO
/****** Object:  StoredProcedure [dbo].[ap_t_RetImport]    Script Date: 04/10/2018 16:06:14 ******/
SET ANSI_NULLS ON
GO
...
BEGIN TRAN
	...
	INSERT dbo.t_PInP (ProdID,PPID,PPDesc,PriceMC_In,PriceMC,Priority,ProdDate,CurrID,CompID,Article,CostAC,PPWeight,File1,File2,File3,PriceCC_In,CostCC,PPDelay,ProdPPDate,DLSDate,IsCommission,CostMC,PriceAC_In,IsCert,FEAProdID,ProdBarCode,PPHumidity,PPImpurity,CustDocNum,CustDocDate)
		...
		join #TempRetImport exc on exc.ProdID = p.ProdID and exc.PPID = p.PPID 
		where not EXISTS ( SELECT * FROM dbo.t_PInP dp where  dp.ProdID = p.ProdID and dp.PPID = p.PPID )
	...
	INSERT dbo.t_Ret 
		...
		PayDelay, CurrID, 0 TSumCC_nt, 0 TTaxSum, 0 TSumCC_wt, 0 TSpendSumCC, 0 TRouteSumCC, StateCode, InDocID, 
