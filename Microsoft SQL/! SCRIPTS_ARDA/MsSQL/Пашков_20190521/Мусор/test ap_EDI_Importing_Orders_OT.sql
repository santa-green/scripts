USE [Alef_Elit]
--GO
--/****** Object:  StoredProcedure [dbo].[ap_EDI_Importing_Orders_OT]    Script Date: 23.01.2019 14:01:07 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO
--ALTER PROCEDURE [dbo].[ap_EDI_Importing_Orders_OT]    
--AS    
--BEGIN  
...
and (exists(select * from dbo.ALEF_EDI_GLN_OT where ZEC_KOD_BASE = ZEO_ZEC_BASE and ZEC_KOD_ADD = ZEO_ZEC_ADD and ZEC_STATUS = 0) 
or exists(select * from dbo.ALEF_EDI_GLN_SETI where EGS_GLN_ID = ZEO_ZEC_BASE and EGS_STATUS = 0)
or exists(select * from dbo.ALEF_EDI_GLN_SETI where EGS_GLN_ID = ZEO_ZEC_ADD and EGS_STATUS = 0))

-- Блокируем задублированные новые заказы (с одинаковым номером заказа). Ставим статус 4 = "Заублированный". 
-- Пропускаем только первый пришедший заказ или отличающиеся по Кол-ву
update dbo.ALEF_EDI_ORDERS_2
set
ZEO_ORDER_STATUS = 4
where ZEO_ORDER_ID in (
	...
	from
		...
		from
			(select ZEO_ORDER_ID, 
			(select SUM(ZEP_POS_QTY) from dbo.ALEF_EDI_ORDERS_2_POS where ZEP_ORDER_ID = ZEO_ORDER_ID) SumQty,
			(select MIN(OO.ZEO_ORDER_ID) from dbo.ALEF_EDI_ORDERS_2 OO where O.ZEO_ORDER_NUMBER = OO.ZEO_ORDER_NUMBER and O.ZEO_ZEC_ADD =  OO.ZEO_ZEC_ADD) MinT
			from dbo.ALEF_EDI_ORDERS_2 as O
...
select 'dbo.ALEF_EXPORT_SUCCESS(ES_SUB, ES_OUR, ES_DATA1, ES_DATA2, ES_DATA3)'
