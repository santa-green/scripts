--USE [Elit]
--GO
--/****** Object:  StoredProcedure [dbo].[ap_Export_J1201009]    Script Date: 22.11.2018 15:08:13 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO
--ALTER PROC [dbo].[ap_Export_J1201009] @ChID INT, @DocCode INT = 0, @IsConsolidated BIT = 0
--AS
--EXEC [dbo].[ap_Export_J1201009] @ChID = 200200009, @DocCode = 11012, @IsConsolidated = 0
DECLARE @ChID INT = 200200009, @DocCode INT = 11012, @IsConsolidated BIT = 0

BEGIN
  SET NOCOUNT ON
  --PRINT @ChID EXEC [dbo].[ap_Export_J1201009] @ChID = 58, @DocCode = 12022, @IsConsolidated = 0
  /*#-BeginDebug Chid
...
-------
  ...
  BEGIN
		...
		WHEN m.CompID=7004 and d.ProdID=26137 THEN 'Віскі Шотл Ханкі Банністер бленд 40% New Design Original 0,7*12 у коробці'  ELSE  p.Notes END) AS 'TAB1_A13', -- по заявке 7630 	
...
------------------------------------------------------------------------
			GROUP BY Cast (d.SrcPosID as varchar(Max)), d.SrcPosID,d.ProdID, m.CodeID2, m.TaxDocDate, LTRIM(RTRIM(ISNULL(NULLIF(ec.ExtProdName,''), p.Notes))), pp.CstProdCode,
		...
		and m.taxdocdate >= '20161201'and p.Article2 like '%&M%' and m.CompID Not between 7000 and 7999 and  m.CompID Not between 10790 and 10799 and r.CodeID4 != 5002  then  p.Article2
		WHEN m.CompID=7004 and d.ProdID=26137 THEN 'Віскі Шотл Ханкі Банністер бленд 40% New Design Original 0,7*12 у коробці'	ELSE p.Notes end),
...
SELECT * FROM @HeadTbl
  ...
  BEGIN    
    --IF EXISTS(SELECT * FROM dbo.at_t_Prepay WHERE ChID = @ChID AND TaxDocDate >= '20150101')
		--BEGIN
			...
			(case when  m.CodeID1 = 63  and m.CodeID2 IN (18,43, 44, 69) and m.CodeID3 = 4   
			and m.taxdocdate >= '20161201'and p.Article2 like '%&M%' and m.CompID Not between 7000 and 7999 and  m.CompID Not between 10790 and 10799 and r.CodeID4 != 5002  then  p.Article2
			else ISNULL(NULLIF(ec.ExtProdName,''), p.Notes)end) AS 'TAB1_A13',
			/*LTRIM(RTRIM(ISNULL(NULLIF(ec.ExtProdName,''), p.Notes))) AS 'TAB1_A13',*/
			rumq.RefName AS 'TAB1_A14',
			REPLICATE(0, 4 - LEN(rumq.RefID)) + CAST(rumq.RefID AS VARCHAR) AS 'TAB1_A141',
			CAST(SUM(Qty) AS NUMERIC(21,6)) AS 'TAB1_A15',
			...
			Join r_Comps r WITH(NOLOCK) on m.CompID = r.CompID
			WHERE m.ChID = @ChID
			GROUP BY Cast(d.SrcPosID as varchar(Max)),d.ProdID, m.CodeID2, m.TaxDocDate, LTRIM(RTRIM(ISNULL(NULLIF(ec.ExtProdName,''), p.Notes))), pp.CstProdCode, 
					rumq.RefName, rumq.RefID, d.PriceCC_nt, d.PriceCC_wt, pp.CstDocCode, pp.CstDocDate,(case when  m.CodeID1 = 63  and m.CodeID2 IN (18,43, 44, 69) and m.CodeID3 = 4   
			and m.taxdocdate >= '20161201'and p.Article2 like '%&M%' and m.CompID Not between 7000 and 7999 and  m.CompID Not between 10790 and 10799 and r.CodeID4 != 5002  then  p.Article2
			else ISNULL(NULLIF(ec.ExtProdName,''), p.Notes)end),
			Case When p.PGrID2 Between 0 and 401 or p.PGrID2  Between 403 and 1000000 THEN pp.CstProdCode end, 
			Case When p.ImpID = 1  THEN 1 end,
			Case When p.PGrID2 = 402 THEN pp.CstProdCode end
  ...
  BEGIN
    ...
    RAISERROR('Отсутствуют данные для экспорта %s J1201009 по %u! Проверьте заполнение основных справочников.', 18, 1, @DocName ,@ChID)
    RETURN
  END
  ...
  SELECT (SELECT 0 AS 'PERTYPE', 
    ...
    FOR XML PATH ('FIELDS'), TYPE
      ...
      (SELECT 
        ...
        UNPIVOT
          (PVal FOR PName IN
            (TAB1_A8,TAB1_A12,TAB1_A13,TAB1_A14,TAB1_A141,TAB1_A15,TAB1_A16,TAB1_A10,TAB1_A19,TAB1_A131,TAB1_A132,TAB1_A133)
          ) AS unpvt
          FOR XML PATH ('ROW'), TYPE
...
END
  ...
  SELECT PStr FROM @tbl ORDER BY ID
	RETURN
	OldProc:
	--  DECLARE @SQL NVARCHAR(255) = 'EXEC [Elit2014].[dbo].[ap_Export_J1201009] @ChID = ' + CAST(@ChID AS VARCHAR) + ', @DocCode = ' + CAST(@ChID AS VARCHAR) + ', @IsConsolidated = ' + CAST(@ChID AS VARCHAR) + 
