...
DECLARE @BDate SMALLDATETIME = '2018-04-16'
DECLARE @EDate SMALLDATETIME = '2018-04-30'
DECLARE @BDateInv SMALLDATETIME = '2017-03-01' --начало периода поиска по РН
DECLARE @EDateInv SMALLDATETIME = '2018-04-15' --конец периода поиска по РН
...
BEGIN
	...
	BEGIN
		...
		--SELECT * FROM #TempFindProdElitAll WITH (NOLOCK) WHERE ProdID_Elit = 30767 ORDER BY rating desc, TQty desc
		...
		BEGIN
			...
			IF (SELECT top 1 QtyRealRet FROM #TempFindProdElit WHERE N = @SrcPosID and ProdID_Elit = @ProdID_Elit) <= 0 
				...
				BEGIN 
					...
					IF (SELECT top 1 N FROM #TempFindProdElit WHERE ProdID_Elit = @ProdID_Elit and TQty <= @TQty and QtyRealRet = 0 ORDER BY TQty DESC ) is not null
					BEGIN
						--распределить по меньшим позициям
						UPDATE #TempFindProdElit 
						SET QtyRealRet = @TQty 
						,CodeID2 = CASE WHEN TQty = @TQty THEN 19 ELSE 44 END --определить признак 2 по позициям
						WHERE N = (SELECT top 1 N FROM #TempFindProdElit WHERE ProdID_Elit = @ProdID_Elit and TQty <= @TQty and QtyRealRet = 0 ORDER BY TQty DESC ) 
					END
					ELSE
						UPDATE #TempFindProdElit 
						SET QtyRealRet = @TQty 
...
--SELECT ChID FROM Elit.dbo.t_Inv WHERE TaxDocID = 358 and TaxDocDate = '2017-06-01'
	...
	INSERT #TempRetFinal
		SELECT ROW_NUMBER()OVER(ORDER BY ProdID_Elit) N, @ProdIDMA ProdIDMA, @ProdIDMA_Qty ProdIDMA_Qty, Price ProdIDMA_RealPrice, 
			...
			NULL vozv_Elit, NULL vozv_ElitDistr, NULL QtyRet, NULL rem, NULL abs_razn, 
			QtyRealRet QtyRealRet, CodeID2 CodeID2, Pos SrcPosID_Inv, NULL Pos_Medoc, NULL sumQtyInv
		FROM #TempFindProdElit 
		WHERE QtyRealRet > 0 and ProdID_Elit = @FindProdID_Elit --32465
...
FOR 
	SELECT TaxDocID, TaxDocDate, ProdID, QtyRealRet,SrcPosID_Inv FROM #TempRetFinal  WITH (NOLOCK) WHERE PPID IS NULL ORDER BY  TaxDocDate,TaxDocID,ProdID,QtyRealRet
OPEN CURSOR3
	FETCH NEXT FROM CURSOR3	INTO @cur3_TaxDocID,@cur3_TaxDocDate,@cur3_ProdID,@cur3_QtyRealRet,@cur3_SrcPosID_Inv
WHILE @@FETCH_STATUS = 0
BEGIN
	--Script
	--IF @cur3_ProdID = 31441
	--BEGIN
	...
	--IF @cur3_ProdID = 31441 SELECT @cur3_PPID cur3_PPID

	IF @cur3_PPID IS NOT NULL
	BEGIN
		--IF @cur3_ProdID = 31441 SELECT @cur3_PPID cur3_PPID_IS_NOT_NULL
		
		UPDATE m 
		SET PPID = @cur3_PPID
		FROM #TempRetFinal m WHERE m.TaxDocID = @cur3_TaxDocID and m.TaxDocDate = @cur3_TaxDocDate and m.ProdID = @cur3_ProdID and m.QtyRealRet = @cur3_QtyRealRet and m.SrcPosID_Inv = @cur3_SrcPosID_Inv  
	END
	ELSE
	BEGIN
		--IF @cur3_ProdID = 31441 SELECT @cur3_PPID cur3_PPID_else
		...
		and t.QtyRealRet = @cur3_QtyRealRet and t.SrcPosID_Inv = @cur3_SrcPosID_Inv 
	END
	
	--IF @cur3_ProdID = 31441 SELECT * FROM #TempRetFinal where ProdID = 31441
...
--SELECT * FROM #TempRetFinal where PPID = 1 --is null
...
--SELECT top 1 ProdIdNabor FROM #ProdIdNabor pn where pn.ProdIDMA = (SELECT top 1 ExtProdID FROM Elit.dbo.r_ProdEC ec where ISNUMERIC(ExtProdID) = 1 and CompID = 10793  and ec.ProdID = 32465)
	...
	BEGIN
		...
		SELECT ProdIDMA, ProdIdNabor, SUM(QtyRet) QtyRet, ProdId, MIN(PriceShop) PriceShop , MIN(Price) Price FROM (
			...
			(SELECT top 1 ProdIdNabor FROM #ProdIdNabor pn where pn.ProdIDMA = (SELECT top 1 ExtProdID FROM Elit.dbo.r_ProdEC ec where ISNUMERIC(ExtProdID) = 1 and CompID = 10793  and ec.ProdID = trf.ProdID)) ProdIdNabor,
...
--для отладки
SELECT COUNT(*) 'итого возвратов' FROM ElitDistr.dbo.t_Ret WHERE ChID >= 4648
SELECT * FROM ElitDistr.dbo.t_Ret WHERE ChID >= 4648
SELECT * FROM ElitDistr.dbo.t_RetD WHERE ChID >= 4648 ORDER BY 1
ROLLBACK
