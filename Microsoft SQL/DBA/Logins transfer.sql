...
SELECT * FROM master.sys.server_role_members SRM
...
SELECT 'USE '+ DB_NAME()+'; '+CASE WHEN dp.state <> 'W' THEN dp.state_desc
                                                        ELSE 'GRANT' END +' ' +
dp.permission_name + ' TO ' + QUOTENAME(dpg.name) COLLATE database_default +
CASE WHEN dp.state <> 'W' THEN ''
                          ELSE ' WITH GRANT OPTION' END +';' AS [-- Permission To Be Assign to the User --]
