...
SELECT to_tsvector('ukrainian', 'Мене звати Влад і я розробник.'); -- SQL Error [42704]: ERROR: text search configuration "ukrainian" does not exist
SELECT to_tsvector('simple', 'Мене звати Влад і я розробник.'); 
SELECT to_tsquery('Hello | world');
SELECT to_tsquery('simple', 'Привіт & світ');
select plainto_tsquery('Hello world');
select phraseto_tsquery('english', 'Hello, world!');
select to_tsvector('Hello, world!') @@ plainto_tsquery('hello world') as match; -- true
select to_tsvector('Hello, worldS!') @@ plainto_tsquery('hello world') as match; -- false
SELECT * FROM docs WHERE to_tsvector('english', body) @@ to_tsquery('english', 'full & text'); --and
SELECT * FROM docs WHERE to_tsvector('english', body) @@ to_tsquery('english', 'full | text'); --or
SELECT * FROM docs WHERE to_tsvector('english', body) @@ to_tsquery('english', 'across <-> town'); --followed by
SELECT to_tsquery('english', 'cat <-> are');   -- cat

select
	id,
	title,
	ts_rank_cd(to_tsvector('english', title) || to_tsvector('english', content), plainto_tsquery('english', 'PostgreSQL & "Full Text Search"')) as rank
from
	articles
where
	to_tsvector('english', title) || to_tsvector('english', content) @@ plainto_tsquery('english', 'PostgreSQL & "Full Text Search"')
order by
	rank desc;
...
select * from docs;
UPDATE docs SET body = body || ' Indexes like GIN help search.' WHERE doc_id = 1 returning *;
...
-- cold:
Seq Scan on docs  (cost=0.00..1.06 rows=1 width=4) (actual time=0.035..0.037 rows=4 loops=1)
  Filter: (sv @@ '''run'' | ''adventur'''::tsquery)
...
/* KEY TAKEAWAYS */
----------------------------------------------------------------------------------------------------
/*What you just exercised (hierarchy, from bits → concept)

Text → tokens → lexemes → tsvector (document representation).

User text → tsquery (to_tsquery / plainto_tsquery / websearch_to_tsquery).

Match with @@, rank with ts_rank, highlight with ts_headline.

Index the tsvector (GIN for fast lookups; GiST is usually faster to update). 
PostgreSQL
+1

Automate with tsvector_update_trigger to keep vectors in sync. 
PostgreSQL
+1*/